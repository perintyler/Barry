...
async function getPackCommands(): Promise<Array<{ name: string; description: string }>> {
  ...
  try {
    ...
    // Pack loading may fail in CI or when packs aren't configured — non-fatal
  }
...
async function buildManifest(): Promise<Record<string, unknown>> {
  const packCommands = await getPackCommands();
  const allCommands = [...BUILTIN_COMMANDS, ...packCommands];
