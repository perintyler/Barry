...
import { loadPackCommands } from "./pack-commands.js";
...
/**
 * Load pack-provided slash commands and merge them into the registry.
...
export async function initCommandHandlers(): Promise<void> {
  ...
  try {
    const packCommands = await loadPackCommands();
    for (const [name, handler] of Object.entries(packCommands)) {
      if (handlers[name]) {
        console.warn(`slack: pack command "${name}" conflicts with built-in — skipping`);
        continue;
      }
      handlers[name] = handler;
      console.warn(`slack: registered pack command /${name}`);
    }
  } catch (err) {
    console.error("slack: failed to load pack commands:", err);
  }
}
