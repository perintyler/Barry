...
import {
  ...
  mapWithConcurrency,
  COOLDOWN_MS,
  CONNECTION_SETTLE_GRACE_MS,
  type BagServerConfig,
  type ConnectedBagProxy,
} from "./bag-proxy.js";

function makeConfig(overrides: Partial<BagServerConfig> & { name: string }): BagServerConfig {
  return { ...overrides };
}

function makeFakeProxy(name: string, toolNames: string[]): ConnectedBagProxy {
  return {
    name,
    client: { close: vi.fn() } as ConnectedBagProxy["client"],
...
describe("poolKey", () => {
  it("keys a normal bag by name and credentials only", () => {
  ...
  // Regression: bag connections are pooled process-wide, so two Barry
  ...
  it("gives a session-scoped bag a distinct key per session", () => {
  ...
  it("falls back to the shared key when a session-scoped bag has no session", () => {
    const config = makeConfig({ name: "browser", sessionScoped: true });
...
describe("retryConnectKey", () => {
  // bag-auth.ts calls pool.clearCooldown(retryConnectKey(config)) right
  // before pool.retryConnect(config) — if the two ever computed different
  // keys, clearCooldown would silently no-op and a just-succeeded auth would
  // still be blocked by a stale cooldown. Pin the exact formula retryConnect
  // itself uses (bagName:credentialHash, no session component).
  it("matches bagName:credentialHash with no session component", () => {
    const config = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
    expect(retryConnectKey(config)).toBe(`notion:${credentialHash(config, {})}`);
  });

  it("defaults resolvedEnv to {} when omitted, matching retryConnect's own default", () => {
    const config = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
    expect(retryConnectKey(config)).toBe(retryConnectKey(config, {}));
  });
});

describe("credentialHash", () => {
  it("returns 'shared' for bags with no env", () => {
  ...
  it("returns a 16-char hex hash for credential bags", () => {
    const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
...
class TestablePool extends BagConnectionPool {
  constructor(
    private _connector: (config: BagServerConfig, env: Record<string, string>) => Promise<ConnectedBagProxy | null>,
  ) {
    super([]);
  }
...
describe("BagConnectionPool", () => {
  ...
  describe("getOrConnect", () => {
    ...
    it("single-flights concurrent calls with the same credentials", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      let resolveConn!: (p: ConnectedBagProxy | null) => void;
    ...
    it("does not single-flight calls with different credentials", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      const resolvers: Array<(p: ConnectedBagProxy | null) => void> = [];
    ...
    it("clears the in-flight slot after failure, but a cooldown gates the retry (astra review F11)", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      connector
      ...
      // Immediately after a failure, a second call must NOT pay the connect
      // timeout again — the cooldown skips it outright, so the connector is
      // never even called a second time. This is the actual F11 fix: a
      // caller hitting a persistently broken bag on every session init used
      // to pay CONNECT_TIMEOUT_MS (45s) every single time.
      expect(await pool.getOrConnect(config, env)).toBeNull();
      expect(connector).toHaveBeenCalledTimes(1);
    });

    it("retries for real once the cooldown is cleared", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      connector
        .mockResolvedValueOnce(null)
        .mockResolvedValueOnce(makeFakeProxy("linear", ["linear_search"]));
      ...
      const env = { LINEAR_API_KEY: "sk-abc" };
      expect(await pool.getOrConnect(config, env)).toBeNull();
      pool.clearCooldown(poolKey(config, env));
      expect(await pool.getOrConnect(config, env)).toHaveLength(1);
      expect(connector).toHaveBeenCalledTimes(2);
    });

    it("retries on its own once the cooldown window elapses", async () => {
      vi.useFakeTimers();
      try {
        const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
        connector
  ...
  describe("retryConnect", () => {
    it("returns enriched tools without caching a pool connection", async () => {
      ...
      // Not cached — shared bags live in pool.shared, and cached entries
      // would be pruned (and their clients closed) by pruneUnused.
      expect(pool.connectionCount).toBe(0);
    });

    it("single-flights concurrent retries for the same bag", async () => {
      const config = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      let resolveConn!: (p: ConnectedBagProxy | null) => void;
      connector.mockImplementation(() => new Promise((r) => { resolveConn = r; }));
    ...
    /**
     * Astra review F11: index.ts's lazy-connect path calls retryConnect on
     * EVERY session init that still needs a failed/needs-auth/auth-expired
     * bag — without a cooldown, a persistently broken bag pays the full
     * CONNECT_TIMEOUT_MS on every single initialize, for every session,
     * until it recovers.
     */
    it("gates a repeat retry behind a cooldown after a failure — the connector is not called again", async () => {
      const config = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      connector
        .mockResolvedValueOnce(null)
        .mockResolvedValueOnce(makeFakeProxy("notion", ["notion_search"]));

      expect(await pool.retryConnect(config)).toBeNull();
      expect(await pool.retryConnect(config)).toBeNull(); // cooldown, not a real attempt
      expect(connector).toHaveBeenCalledTimes(1);
    });

    it("bag-auth's clearCooldown(retryConnectKey(config)) unblocks the very next retryConnect call", async () => {
      const config = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      connector
        .mockResolvedValueOnce(null)
        .mockResolvedValueOnce(makeFakeProxy("notion", ["notion_search"]));
      ...
      expect(await pool.retryConnect(config)).toBeNull();
      pool.clearCooldown(retryConnectKey(config));
      expect(await pool.retryConnect(config)).toHaveLength(1);
      expect(connector).toHaveBeenCalledTimes(2);
    });
  });

  describe("getToolsForSession", () => {
    it("returns shared tools plus credential bag tools", async () => {
      const linearConfig = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      pool.deferredConfigs["linear"] = linearConfig;
      pool.shared = [
        { name: "vault_status", description: "", inputSchema: {}, bag: "vault", namespace: "vault", access: "read", proxy: {} as ConnectedBagProxy },
      ];
    ...
    it("skips bags not in deferredConfigs", async () => {
      pool.shared = [];
    ...
    it("handles failed bag connections gracefully", async () => {
  ...
  describe("getActiveKeys", () => {
    it("returns cache keys for deferred bags", () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
    ...
    it("excludes bags not in deferredConfigs", () => {
  ...
  describe("pruneUnused", () => {
    it("closes connections not in the active set, once past the settle grace", async () => {
      vi.useFakeTimers();
      try {
        const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
        const proxy = makeFakeProxy("linear", ["linear_search"]);
        ...
        await vi.advanceTimersByTimeAsync(CONNECTION_SETTLE_GRACE_MS + 1);
    ...
    it("prunes selectively when some connections are active, once past the settle grace", async () => {
      vi.useFakeTimers();
      try {
        const linearConfig = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
    ...
    /**
     * Astra review F9-F11: the actual regression this pool-level fix closes.
     * index.ts's caller-supplied `activeKeys` only reflects transports that
     * finished registering — a transport still mid-initialize (past its own
     * connect, not yet at registerTransport) looks completely unreferenced
     * to a sweep that fires in that gap. Without the grace period, THIS test
     * would show connectionCount going to 0 immediately.
     */
    it("does NOT prune a freshly-connected key even with an empty active set — the settle grace", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      const proxy = makeFakeProxy("linear", ["linear_search"]);
      connector.mockResolvedValue(proxy);
      ...
      // No time has passed and activeKeys is empty — old code would prune
      // this immediately, closing a connection that was JUST established.
      pool.pruneUnused(new Set());
  ...
  describe("needsAuth recovery", () => {
    it("needsAuth bags are not included in getToolsForSession", async () => {
      ...
      // needsAuth bags are not in deferredConfigs, so getToolsForSession skips them
      expect(tools).toEqual([]);
      expect(connector).not.toHaveBeenCalled();
    });

    it("needsAuth bags can be moved to shared after successful reconnection", async () => {
      const notionConfig = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      pool.needsAuth["notion"] = notionConfig;
      pool.shared = [];
    ...
    it("needsAuth bags stay in needsAuth if reconnection fails", async () => {
      const notionConfig = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      pool.needsAuth["notion"] = notionConfig;
      ...
      // Bag stays in needsAuth — not removed on failure
    ...
    it("failedSharedConfigs and needsAuth can be retried together", async () => {
      ...
      const bagsToRetry = ["notion", "sentry"]
        .filter((ns) => retryPools[ns])
        .map((ns) => retryPools[ns]);

      for (const config of bagsToRetry) {
  ...
  describe("concurrent multi-barry scenarios", () => {
    it("two identities with different credentials for the same bag get separate connections", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      pool.deferredConfigs["linear"] = config;
    ...
    it("two sessions with the same barry reuse the same connection", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      pool.deferredConfigs["linear"] = config;
    ...
    it("no-barry session only gets shared tools", async () => {
      pool.deferredConfigs["linear"] = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      pool.shared = [
        { name: "playwright_click", description: "", inputSchema: {}, bag: "playwright", namespace: "playwright", access: "write", proxy: {} as ConnectedBagProxy },
      ];
    ...
    it("cleanup after all sessions end prunes everything, once past the settle grace", async () => {
      vi.useFakeTimers();
      try {
        const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
        pool.deferredConfigs["linear"] = config;
  ...
  describe("markAuthExpired", () => {
    ...
    it("does nothing for unknown bags", () => {
  ...
  describe("isBagOAuth", () => {
    it("returns true for URL-only non-localhost bags", () => {
      pool.originalConfigs["notion"] = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      expect(pool.isBagOAuth("notion")).toBe(true);
    });

    it("returns false for bags with env vars", () => {
      pool.originalConfigs["linear"] = makeConfig({ name: "linear", url: "https://linear.com/mcp", env: ["LINEAR_API_KEY"] });
      expect(pool.isBagOAuth("linear")).toBe(false);
    });

    it("returns false for localhost bags", () => {
      pool.originalConfigs["vault"] = makeConfig({ name: "vault", url: "http://localhost:3923/mcp" });
      expect(pool.isBagOAuth("vault")).toBe(false);
    });

    it("returns false for unknown bags", () => {
      expect(pool.isBagOAuth("nope")).toBe(false);
    });
  });

  describe("authExpired bags participate in retry", () => {
    it("authExpired bags can be moved back to shared after successful reconnection", async () => {
      const sentryConfig = makeConfig({ name: "sentry", url: "https://mcp.sentry.dev/mcp" });
...
describe("mapWithConcurrency", () => {
  ...
  it("runs every item even when the limit exceeds the item count", async () => {
    const calls: number[] = [];
    await mapWithConcurrency([1, 2, 3], 100, async (n) => { calls.push(n); });
...
// Regression: upstream servers report tool failure by RESOLVING with
// `isError: true` rather than throwing, so `client.callTool` returns normally
// and the proxy's catch block never runs. This is how a Notion `invalid_token`
// reached the agent verbatim, with no instruction to call `bag_auth`.
describe("inBandErrorText", () => {
  it("extracts text from a result flagged isError", () => {
    expect(
      inBandErrorText({
        isError: true,
        content: [{ type: "text", text: '{"error":"invalid_token"}' }],
      }),
    ).toBe('{"error":"invalid_token"}');
  });
    ...
    expect(
      inBandErrorText({
        ...
        content: [
          ...
          { type: "image", data: "…" },
          { type: "text", text: "token expired" },
        ],
      }),
    ).toBe("Unauthorized\ntoken expired");
  });

  it("returns undefined for successful results so they pass through", () => {
    expect(inBandErrorText({ content: [{ type: "text", text: "ok" }] })).toBeUndefined();
    expect(inBandErrorText({ isError: false, content: [] })).toBeUndefined();
    expect(inBandErrorText(null)).toBeUndefined();
    expect(inBandErrorText("nope")).toBeUndefined();
  });

  it("returns empty string when flagged isError with no usable content", () => {
    expect(inBandErrorText({ isError: true })).toBe("");
  });

  // The two bugs compound: extracting the text is only useful if the pattern
  // then matches it. This asserts the end-to-end path the Notion case takes.
  it("feeds an in-band auth error into isAuthError", () => {
    const text = inBandErrorText({
      isError: true,
      content: [{ type: "text", text: '{"error":"invalid_token"}' }],
    });
    expect(text).toBeDefined();
    expect(isAuthError(text as string)).toBe(true);
  });
});
...
describe("isAuthError", () => {
  ...
  // Regression: upstream servers send RFC 6749/6750 error CODES, not prose.
  // `/invalid token/` (space) cannot match `invalid_token` (underscore), so the
  // most standard spelling was the one that slipped through to the agent.
  it("matches underscored OAuth error codes", () => {
    expect(isAuthError("invalid_token")).toBe(true);
    expect(isAuthError('{"error":"invalid_token"}')).toBe(true);
    expect(isAuthError('{"error":"invalid_grant"}')).toBe(true);
    expect(isAuthError('{"error":"invalid_client"}')).toBe(true);
    expect(isAuthError("token_expired")).toBe(true);
    expect(isAuthError("invalid-api-key")).toBe(true);
  });

  it("matches 'token is invalid' phrasing", () => {
    expect(isAuthError("API token is invalid.")).toBe(true);
  });
  ...
  describe("transport death patterns", () => {
    it("matches transport closed when isOAuthBag=true", () => {
      expect(isAuthError("transport closed unexpectedly", true)).toBe(true);
    });

    it("matches ECONNREFUSED when isOAuthBag=true", () => {
      expect(isAuthError("connect ECONNREFUSED 127.0.0.1:3000", true)).toBe(true);
    });

    it("matches process exit when isOAuthBag=true", () => {
      expect(isAuthError("process exit code 1", true)).toBe(true);
    });

    it("does NOT match transport errors when isOAuthBag=false", () => {
...
describe("authExpiredMessage", () => {
  it("includes bag name and fix command", () => {
    const msg = authExpiredMessage("notion");
    expect(msg).toContain('"notion"');
    expect(msg).toContain("barry bag auth notion");
  });

  it("instructs the agent to call the bag_auth tool", () => {
    const msg = authExpiredMessage("notion");
    expect(msg).toContain("bag_auth");
  });
  ...
  // A non-OAuth (API-key / keyless) bag must NOT be pointed at bag_auth — it
  // answers such bags with "does not use OAuth", so the remedy would be a
  // dead end. Regression: the keyless web-fetching bag hit an upstream
  // 401/quota error and agents chased bag_auth on this message's advice.
  it("non-OAuth bags are pointed at credentials, never at bag_auth", () => {
    const msg = authExpiredMessage("web-fetching", "429 rate limit / unauthorized", false);
    expect(msg).not.toContain('call the `bag_auth` tool');
    expect(msg).not.toContain("barry bag auth");
    expect(msg).toContain("barry vault set-env");
    expect(msg).toContain("Original error: 429 rate limit / unauthorized");
  });

  it("isOAuth defaults to the OAuth wording (existing call sites unchanged)", () => {
    expect(authExpiredMessage("notion")).toContain("bag_auth");
  });
});

// connectBagProxy — token gate + timeout cleanup (mocked SDK client)

describe("connectBagProxy", () => {
  beforeEach(() => {
    h.hasTokens = false;
    h.httpBehavior = "ok";
    h.stdioBehavior = "ok";
  ...
  it("connects URL-only bags via direct HTTP when possible", async () => {
    const result = await connectBagProxy(makeConfig({ name: "vault", url: "http://localhost:3923/mcp" }));
  ...
  it("does NOT spawn mcp-remote when direct HTTP fails and no cached tokens exist", async () => {
    ...
    const result = await connectBagProxy(makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" }));
  ...
  it("closes created clients on timeout so spawned children are not leaked", async () => {
    ...
    const pending = connectBagProxy(makeConfig({ name: "slow", url: "https://example.com/mcp" }));
...
describe("addSharedToolsForBag", () => {
  it("replaces existing entries for the bag instead of appending duplicates", () => {
    const pool = new BagConnectionPool([]);
    ...
    addSharedToolsForBag(pool, "notion", newTools);

    expect(pool.shared).toHaveLength(3);
    expect(pool.shared.filter((t) => t.bag === "notion")).toHaveLength(2);
    expect(pool.shared.filter((t) => t.bag === "vault")).toHaveLength(1);
  ...
  it("closes the superseded proxy instead of orphaning its process", () => {
    // pool.shared holds the ONLY reference to an eagerly-connected proxy —
    // shared bags never enter `connections`, so pruneUnused cannot reach
    // them. Replacing the tools without closing stranded the child process
    // (an npx/uvx server) for the life of the MCP server, once per retry.
    const pool = new BagConnectionPool([]);
    const oldProxy = makeFakeProxy("notion", ["notion_search"]);
    const otherProxy = makeFakeProxy("vault", ["vault_status"]);
    pool.shared = [
      ...enrichProxiedTools([oldProxy], []),
      ...enrichProxiedTools([otherProxy], []),
    ];

    const newProxy = makeFakeProxy("notion", ["notion_search"]);
    addSharedToolsForBag(pool, "notion", enrichProxiedTools([newProxy], []));

    expect(oldProxy.client.close).toHaveBeenCalledTimes(1);
    // An unrelated bag must not be collateral damage.
    expect(otherProxy.client.close).not.toHaveBeenCalled();
    // The incoming connection is the live one — closing it would be a self-kill.
    expect(newProxy.client.close).not.toHaveBeenCalled();
  });

  it("does not close a proxy that is being re-added unchanged", () => {
    // bag-auth retries can hand back the same live proxy. Excluding `keep`
    // makes that a no-op rather than disconnecting the connection in use.
    const pool = new BagConnectionPool([]);
    const proxy = makeFakeProxy("notion", ["notion_search"]);
    const tools = enrichProxiedTools([proxy], []);
    pool.shared = [...tools];

    addSharedToolsForBag(pool, "notion", tools);
...
describe("markAuthExpired", () => {
  it("closes the shared proxy rather than just dropping its tools", () => {
    // Same leak as above on the auth-expiry path: filtering pool.shared
    // discarded the last handle on the child process without closing it.
    const pool = new BagConnectionPool([]);
    const proxy = makeFakeProxy("notion", ["notion_search"]);
    pool.shared = [...enrichProxiedTools([proxy], [])];
    pool.connectedSharedBags.add("notion");
    pool.originalConfigs.notion = makeConfig({ name: "notion", url: "https://example.com/mcp" });

    pool.markAuthExpired("notion");

    expect(proxy.client.close).toHaveBeenCalledTimes(1);
    expect(pool.shared.filter((t) => t.bag === "notion")).toHaveLength(0);
    expect(pool.connectedSharedBags.has("notion")).toBe(false);
  });
});

describe("pool deferredBagNames threading", () => {
  it("getOrConnect stamps deferred when pool.deferredBagNames includes the bag", async () => {
    const connector = vi.fn();
    const pool = new TestablePool(connector);
    pool.deferredBagNames = new Set(["linear"]);
  ...
  it("getOrConnect leaves tools un-deferred when bag is not in deferredBagNames", async () => {
    const connector = vi.fn();
    const pool = new TestablePool(connector);
    pool.deferredBagNames = new Set(["notion"]);
...
describe("enrichProxiedTools deferral", () => {
  it("stamps deferred=true on tools whose bag is in deferredBags", () => {
  ...
  it("leaves tools not in deferredBags un-deferred", () => {
    const tools = enrichProxiedTools(
  ...
  it("defaults deferred=false when no deferredBags set is passed (back-compat)", () => {
...
describe("zodSchemaToJson", () => {
  ...
  it("converts an empty schema stored by the MCP SDK as a zod-mini object", async () => {
    ...
    const stored = getRegisteredTools(server)["no_args"].inputSchema;
  ...
  it("keeps parameters for a non-empty zod v4 shape stored as a zod-mini object", async () => {
    ...
    const stored = getRegisteredTools(server)["v4_args"].inputSchema;
  ...
  // Regression: bags resolve their own zod copy, so `instanceof z.ZodObject`
  // was false for bag-built schemas. The schema fell through to z.object(),
  ...
  // bag pins (3.25.76) so the schema is cross-realm for real, rather than
  // faked by breaking its prototype. Bags all pin 3.25.76, so this is exactly
  ...
  // zod-to-json-schema is v3-only. Every bag pins zod 3.x today, so this is a
...
// patchServerWithProxiedTools -- mailbox delivery

describe("patchServerWithProxiedTools mailbox delivery", () => {
  beforeEach(() => {
    vi.resetModules();
  });

  it("appends a queued phone message to the next tool call's result", async () => {
    vi.doMock("@barry-rocks/db", () => ({
      popPrompts: vi.fn().mockResolvedValue([
        { id: "p1", session_id: "s1", content: "hi from the phone", created_at: "2026-01-01T00:00:00Z" },
      ]),
    }));

    const { McpServer } = await import("@modelcontextprotocol/sdk/server/mcp.js");
    const { getRequestHandler } = await import("./sdk-internals.js");
    const { patchServerWithProxiedTools } = await import("./bag-proxy.js");

    const server = new McpServer({ name: "t", version: "1" });
    server.tool("echo", "echoes", {}, async () => ({ content: [{ type: "text", text: "original" }] }));

    patchServerWithProxiedTools(server, [], undefined, { force: true, plannedSessionId: "s1" });

    const handler = getRequestHandler(server.server, "tools/call");
    expect(handler).toBeDefined();

    const result = await handler!({ method: "tools/call", params: { name: "echo", arguments: {} } }, {}) as {
    ...
    expect(result.content[1].text).toContain("phone-message");
    expect(result.content[1].text).toContain("hi from the phone");
  });

  it("leaves the result untouched when no prompts are queued", async () => {
    vi.doMock("@barry-rocks/db", () => ({
      popPrompts: vi.fn().mockResolvedValue([]),
    }));
  ...
  it("never checks the mailbox when no session is attached", async () => {
    const popPrompts = vi.fn();
    vi.doMock("@barry-rocks/db", () => ({ popPrompts }));
    ...
    patchServerWithProxiedTools(server, [], undefined, { force: true });

    const handler = getRequestHandler(server.server, "tools/call");
    await handler!({ method: "tools/call", params: { name: "echo", arguments: {} } }, {});

    expect(popPrompts).not.toHaveBeenCalled();
  });
});
