...
vi.mock("@barry/blocks", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@barry/blocks")>();
  return {
    ...actual,
...
import { authenticateBlock, resetBlockAuthState } from "./block-auth.js";
import type { BlockConnectionPool, FilterableProxiedTool } from "./block-proxy.js";
import type { Block } from "@barry/blocks";

function makeTool(block: string, name: string): FilterableProxiedTool {
  return {
    name,
    description: "",
    inputSchema: {},
...
type FakePool = BlockConnectionPool & { retryConnect: ReturnType<typeof vi.fn> };
...
describe("authenticateBlock", () => {
  ...
  it("fails without spawning for unknown blocks", async () => {
    const pool = makePool();
    const result = await authenticateBlock(pool, "nope");

    expect(result.ok).toBe(false);
    expect(result.message).toContain("Unknown block");
    expect(h.children).toHaveLength(0);
  });

  it("fails without spawning for blocks with no MCP server URL", async () => {
    const pool = makePool();
    const result = await authenticateBlock(pool, "local-only");
  ...
  it("API-key block: never spawns, reports missing keys when unset", async () => {
    vi.stubEnv("DD_API_KEY", "");
    vi.stubEnv("DD_APP_KEY", "");
    const pool = makePool();

    const result = await authenticateBlock(pool, "datadog-remote");
  ...
  it("API-key block: never spawns, says keys are set when present", async () => {
  ...
  it("success path: one OAuth flow, reconnect, buckets cleared, shared repopulated", async () => {
    ...
    const pending = authenticateBlock(pool, "notion");
  ...
  it("single-flight: concurrent calls share one attempt and one spawn", async () => {
    ...
    const p1 = authenticateBlock(pool, "notion");
    const p2 = authenticateBlock(pool, "notion");
  ...
  it("cooldown blocks an immediate retry after failure — no second tab", async () => {
    ...
    const retry = await authenticateBlock(pool, "notion");
  ...
  it("allows a fresh attempt after the cooldown expires", async () => {
    ...
    const first = authenticateBlock(pool, "notion");
    await vi.advanceTimersByTimeAsync(120_000);
    await first;
    expect(h.children).toHaveLength(1);
    ...
    const second = authenticateBlock(pool, "notion");
    h.hasTokens = true;
    await vi.advanceTimersByTimeAsync(2_000);
  ...
  function makeCliBlock(overrides?: Partial<Block["manifest"]>): Block {
    return {
      name: "temporal",
      description: "Temporal block",
      builtin: false,
      source: { type: "local", path: "/repos/blocks/temporal" },
      manifest: {
        manifestVersion: 1,
        name: "temporal",
  ...
  it("CLI-delegated: returns ok immediately when already authenticated", async () => {
    const pool = makePool();
    const blocks = [makeCliBlock()];
    h.execFileResults.set("temporal cloud whoami", 0);

    const result = await authenticateBlock(pool, "temporal", blocks);
  ...
  it("CLI-delegated: spawns login command and polls check until success", async () => {
    ...
    const pending = authenticateBlock(pool, "temporal", blocks);
  ...
  it("CLI-delegated: times out when check never passes", async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.message).toContain("did not complete");
    expect(result.message).toContain("barry block auth temporal");
  });
  ...
  it("CLI-delegated: cooldown blocks immediate retry after timeout", async () => {
    ...
    const first = authenticateBlock(pool, "temporal", blocks);
    await vi.advanceTimersByTimeAsync(120_000);
    await first;

    const retry = await authenticateBlock(pool, "temporal", blocks);
    expect(retry.ok).toBe(false);
    expect(retry.message).toContain("failed less than a minute ago");
  });

  it("CLI-delegated: retries failed block connection after auth succeeds", async () => {
    const pool = makePool();
  ...
  it("CLI-delegated: works without a check command (fire-and-forget)", async () => {
    const pool = makePool();
    const blocks = [makeCliBlock({ auth: { command: "vendor", args: ["login"] } })];
  ...
  it("CLI-delegated: takes priority over OAuth path for blocks with auth manifest", async () => {
    // Even if the block also has an MCP server URL in the pool, the manifest
