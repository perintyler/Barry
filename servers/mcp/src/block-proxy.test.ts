...
function makeConfig(overrides: Partial<BlockServerConfig> & { name: string }): BlockServerConfig {
  return { ...overrides };
}

function makeFakeProxy(name: string, toolNames: string[]): ConnectedBlockProxy {
  return {
    name,
    client: { close: vi.fn() } as unknown as ConnectedBlockProxy["client"],
...
describe("poolKey", () => {
  it("keys a normal block by name and credentials only", () => {
  ...
  // Regression: block connections are pooled process-wide, so two Barry
  ...
  it("gives a session-scoped block a distinct key per session", () => {
  ...
  it("falls back to the shared key when a session-scoped block has no session", () => {
    const config = makeConfig({ name: "browser", sessionScoped: true });
...
describe("credentialHash", () => {
  it("returns 'shared' for blocks with no env", () => {
  ...
  it("returns a 16-char hex hash for credential blocks", () => {
    const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
...
class TestablePool extends BlockConnectionPool {
  constructor(
    private _connector: (config: BlockServerConfig, env: Record<string, string>) => Promise<ConnectedBlockProxy | null>,
  ) {
    super([]);
  }
...
describe("BlockConnectionPool", () => {
  ...
  describe("getOrConnect", () => {
    ...
    it("single-flights concurrent calls with the same credentials", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      let resolveConn!: (p: ConnectedBlockProxy | null) => void;
    ...
    it("does not single-flight calls with different credentials", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      const resolvers: Array<(p: ConnectedBlockProxy | null) => void> = [];
  ...
  describe("retryConnect", () => {
    it("returns enriched tools without caching a pool connection", async () => {
      ...
      // Not cached — shared blocks live in pool.shared, and cached entries
      // would be pruned (and their clients closed) by pruneUnused.
      expect(pool.connectionCount).toBe(0);
    });

    it("single-flights concurrent retries for the same block", async () => {
      const config = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      let resolveConn!: (p: ConnectedBlockProxy | null) => void;
      connector.mockImplementation(() => new Promise((r) => { resolveConn = r; }));
  ...
  describe("getToolsForSession", () => {
    it("returns shared tools plus credential block tools", async () => {
      const linearConfig = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      pool.deferredConfigs["linear"] = linearConfig;
      pool.shared = [
        { name: "vault_status", description: "", inputSchema: {}, block: "vault", namespace: "vault", access: "read", proxy: {} as unknown as ConnectedBlockProxy },
      ];
    ...
    it("skips blocks not in deferredConfigs", async () => {
      pool.shared = [];
    ...
    it("handles failed block connections gracefully", async () => {
  ...
  describe("getActiveKeys", () => {
    it("returns cache keys for deferred blocks", () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
    ...
    it("excludes blocks not in deferredConfigs", () => {
  ...
  describe("needsAuth recovery", () => {
    it("needsAuth blocks are not included in getToolsForSession", async () => {
      ...
      // needsAuth blocks are not in deferredConfigs, so getToolsForSession skips them
      expect(tools).toEqual([]);
      expect(connector).not.toHaveBeenCalled();
    });

    it("needsAuth blocks can be moved to shared after successful reconnection", async () => {
      const notionConfig = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      pool.needsAuth["notion"] = notionConfig;
      pool.shared = [];
    ...
    it("needsAuth blocks stay in needsAuth if reconnection fails", async () => {
      const notionConfig = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      pool.needsAuth["notion"] = notionConfig;
      ...
      // Block stays in needsAuth — not removed on failure
    ...
    it("failedSharedConfigs and needsAuth can be retried together", async () => {
      ...
      const blocksToRetry = ["notion", "sentry"]
        .filter((ns) => retryPools[ns])
        .map((ns) => retryPools[ns]);

      for (const config of blocksToRetry) {
  ...
  describe("concurrent multi-barry scenarios", () => {
    it("two identities with different credentials for the same block get separate connections", async () => {
      const config = makeConfig({ name: "linear", env: ["LINEAR_API_KEY"] });
      pool.deferredConfigs["linear"] = config;
    ...
    it("no-barry session only gets shared tools", async () => {
      ...
      pool.shared = [
        { name: "playwright_click", description: "", inputSchema: {}, block: "playwright", namespace: "playwright", access: "write", proxy: {} as unknown as ConnectedBlockProxy },
      ];
  ...
  describe("markAuthExpired", () => {
    ...
    it("does nothing for unknown blocks", () => {
      pool.markAuthExpired("nonexistent");
      expect(pool.authExpired).not.toHaveProperty("nonexistent");
    });
  });

  describe("isBlockOAuth", () => {
    it("returns true for URL-only non-localhost blocks", () => {
      pool.originalConfigs["notion"] = makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" });
      expect(pool.isBlockOAuth("notion")).toBe(true);
    });

    it("returns false for blocks with env vars", () => {
      pool.originalConfigs["linear"] = makeConfig({ name: "linear", url: "https://linear.com/mcp", env: ["LINEAR_API_KEY"] });
      expect(pool.isBlockOAuth("linear")).toBe(false);
    });

    it("returns false for localhost blocks", () => {
      pool.originalConfigs["vault"] = makeConfig({ name: "vault", url: "http://localhost:3923/mcp" });
      expect(pool.isBlockOAuth("vault")).toBe(false);
    });

    it("returns false for unknown blocks", () => {
      expect(pool.isBlockOAuth("nope")).toBe(false);
    });
  });

  describe("authExpired blocks participate in retry", () => {
    it("authExpired blocks can be moved back to shared after successful reconnection", async () => {
      const sentryConfig = makeConfig({ name: "sentry", url: "https://mcp.sentry.dev/mcp" });
...
describe("isAuthError", () => {
  ...
  describe("transport death patterns", () => {
    it("matches transport closed when isOAuthBlock=true", () => {
      expect(isAuthError("transport closed unexpectedly", true)).toBe(true);
    });

    it("matches ECONNREFUSED when isOAuthBlock=true", () => {
      expect(isAuthError("connect ECONNREFUSED 127.0.0.1:3000", true)).toBe(true);
    });

    it("matches process exit when isOAuthBlock=true", () => {
      expect(isAuthError("process exit code 1", true)).toBe(true);
    });

    it("does NOT match transport errors when isOAuthBlock=false", () => {
...
describe("authExpiredMessage", () => {
  it("includes block name and fix command", () => {
    const msg = authExpiredMessage("notion");
    expect(msg).toContain('"notion"');
    expect(msg).toContain("barry block auth notion");
  });

  it("instructs the agent to call the block_auth tool", () => {
    const msg = authExpiredMessage("notion");
    expect(msg).toContain("block_auth");
  });
...
// --------------------------------------------------------------------------
// connectBlockProxy — token gate + timeout cleanup (mocked SDK client)
// --------------------------------------------------------------------------

describe("connectBlockProxy", () => {
  beforeEach(() => {
    h.hasTokens = false;
    h.httpBehavior = "ok";
    h.stdioBehavior = "ok";
  ...
  it("connects URL-only blocks via direct HTTP when possible", async () => {
    const result = await connectBlockProxy(makeConfig({ name: "vault", url: "http://localhost:3923/mcp" }));
  ...
  it("does NOT spawn mcp-remote when direct HTTP fails and no cached tokens exist", async () => {
    ...
    const result = await connectBlockProxy(makeConfig({ name: "notion", url: "https://mcp.notion.com/mcp" }));
  ...
  it("closes created clients on timeout so spawned children are not leaked", async () => {
    ...
    const pending = connectBlockProxy(makeConfig({ name: "slow", url: "https://example.com/mcp" }));
...
describe("addSharedToolsForBlock", () => {
  it("replaces existing entries for the block instead of appending duplicates", () => {
    const pool = new BlockConnectionPool([]);
    ...
    addSharedToolsForBlock(pool, "notion", newTools);

    expect(pool.shared).toHaveLength(3);
    expect(pool.shared.filter((t) => t.block === "notion")).toHaveLength(2);
    expect(pool.shared.filter((t) => t.block === "vault")).toHaveLength(1);
...
describe("pool deferredBlockNames threading", () => {
  it("getOrConnect stamps deferred when pool.deferredBlockNames includes the block", async () => {
    const connector = vi.fn();
    const pool = new TestablePool(connector);
    pool.deferredBlockNames = new Set(["linear"]);
  ...
  it("getOrConnect leaves tools un-deferred when block is not in deferredBlockNames", async () => {
    const connector = vi.fn();
    const pool = new TestablePool(connector);
    pool.deferredBlockNames = new Set(["notion"]);
...
describe("enrichProxiedTools deferral", () => {
  it("stamps deferred=true on tools whose block is in deferredBlocks", () => {
  ...
  it("leaves tools not in deferredBlocks un-deferred", () => {
    const tools = enrichProxiedTools(
  ...
  it("defaults deferred=false when no deferredBlocks set is passed (back-compat)", () => {
...
describe("zodSchemaToJson", () => {
  ...
  // Regression: blocks resolve their own zod copy, so `instanceof z.ZodObject`
  // was false for block-built schemas. The schema fell through to z.object(),
  ...
  // block pins (3.25.76) so the schema is cross-realm for real, rather than
  // faked by breaking its prototype. Blocks all pin 3.25.76, so this is exactly
  ...
  // zod-to-json-schema is v3-only. Every block pins zod 3.x today, so this is a
