...
- **Session scope**: if the request names a barry session id (live-review nudges
  do), call `bdiff_list_comments` with that `sessionId`. Session comments may
  span multiple repos.
- **Repo scope**: otherwise use the repo path — the current working directory
  unless the user names a different repo — and call `bdiff_list_comments` with
  that absolute path.
...
3. **Understand the request.** Read `body` and any `replies`. If the comment is unclear, or you disagree with it, call `bdiff_reply_comment` explaining why and leave it open — do **not** resolve it.
...
5. **Resolve.** Call `bdiff_resolve_comment` with a 1–3 sentence note: what changed, where, and how it was verified. If the code was already fixed or removed, resolve with a note saying so.
