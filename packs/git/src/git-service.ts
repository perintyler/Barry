...
export class GitService {
  ...
  async commit(repoPath: string, message: string, options: { all?: boolean } = {}): Promise<string> {
    const args = ["commit"];
    if (options.all) {
      args.push("-a");
    }
  ...
  async push(repoPath: string, options: { remote?: string; branch?: string; setUpstream?: boolean } = {}): Promise<string> {
    const args = ["push"];
