...
export const getCurrentSession = defineTool({
  ...
  description: "Get the current session using the BARRY_SESSION_ID environment variable. Returns the session you are currently running in.",
  schema: {},
  handler: async () => {
    const session = await getService().getCurrentSession();
    if (!session) throw new Error("No current session (BARRY_SESSION_ID not set or session not found)");
    return session;
  },
});
