...
pack owns the storage, the path keying, and two read-only views.
...
```bash
cd ~/repos/barry/packs/locks && npx tsc --noEmit
```
...
```bash
cd ~/repos/barry/packs/locks && pnpm test 2>&1 | tail -5
```
...
```bash
cd ~/repos/barry/packs/locks && npx tsx -e "
import('/Users/tyler/repos/barry/packs/locks/src/tools.js').then(m => {
...
```bash
cd ~/repos/barry/packs/locks && cat > /tmp/locks-qa-run.ts <<'EOF'
import { acquireForEdit } from "/Users/tyler/repos/barry/packs/locks/src/acquire.js";
import { locksContention } from "/Users/tyler/repos/barry/packs/locks/src/tools.js";
...
cd ~/repos/barry/packs/locks && npx tsx -e "
import('/Users/tyler/repos/barry/packs/locks/src/acquire.js').then(({ acquireForEdit }) =>
...
```bash
cd ~/repos/barry/packs/locks && npx tsx -e "
import { parseManifest, getSkillsDirs } from '@barry/packs';
const m = parseManifest('/Users/tyler/repos/barry/packs/locks');
console.log(m.name, '| tools:', m.toolsEntry?.entry, '| deferred:', (m.toolsEntry?.deferred ?? []).join(','), '| skills:', getSkillsDirs('/Users/tyler/repos/barry/packs/locks').length);
" 2>&1 | grep -v DEP0205
```
...
```bash
test -f ~/repos/barry/packs/locks/skills/resolve-lock-conflict/SKILL.md && sed -n '2p' ~/repos/barry/packs/locks/skills/resolve-lock-conflict/SKILL.md
```
...
import('/Users/tyler/repos/barry/packs/locks/src/repo.js').then(({toRepoPath})=>{
...
```bash
cd ~/repos/barry && npx tsx -e "
import('/Users/tyler/repos/barry/packs/locks/src/tools.js').then(async m => {
...
### 15. Pack resolves through the CLI

```bash
barry pack show locks 2>&1 | head -14
```
...
- [ ] `tsc --noEmit` is clean
- [ ] Unit tests pass (pack + `wrapEditIntent` suite)
