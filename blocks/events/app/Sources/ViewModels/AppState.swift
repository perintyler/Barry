...
final class AppState: ObservableObject {
    ...
    func open(_ event: BarryEvent) {
        ...
        // identically.
        if case .string("block_auth") = event.data["action"] {
            let blocks = event.authBlocks
            if !blocks.isEmpty {
                Task { await authorizeBlocks(blocks) }
                return
            }
        }
    ...
    /// Start the OAuth flow for blocks that need it.
    ///
    ...
    /// is single-flight per block, so repeated clicks can't stack up tabs.
    func authorizeBlocks(_ blocks: [String]) async {
        for block in blocks {
            await client.startBlockAuth(block)
        }
    }
