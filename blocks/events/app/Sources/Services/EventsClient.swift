...
struct EventsClient {
    ...
    /// Ask the API to begin OAuth for a block. Best-effort: the browser tab is
    /// the real feedback, and the endpoint is idempotent while an attempt is
    /// already pending.
    func startBlockAuth(_ block: String) async {
        let url = baseURL.appendingPathComponent("api/v1/identities/blocks/\(block)/auth")
        try? await post(url)
    }
