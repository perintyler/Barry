...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    // authorizing a block, or opening its session in the web UI.
    nonisolated func userNotificationCenter(
        _ center: UNUserNotificationCenter,
        ...
        if info["action"] as? String == "block_auth", let blocks = info["blocks"] as? [String] {
            // Kicks off the OAuth flow server-side; mcp-remote opens the browser
            // tab. The endpoint is single-flight per block, so a double-click on
            // the banner can't produce two tabs.
            Task { @MainActor in
                await self.state.authorizeBlocks(blocks)
                completionHandler()
            }
            return
        }
