...
1. Use the `status` tool and read diffs (`diff`) to understand all changes
...
3. Execute each group with the `add` tool (stage the group's files) then the `commit` tool (with the message)
4. Show the result with the `log` tool
