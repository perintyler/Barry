    ...
    private var scopePicker: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text(editor.isFileBased
                     ? "Applied to every session using this identity. Picking one copies its rules into identity.yaml, which has no way to reference a scope by name."
                     : "Applied to every session using this identity. Merges with any session-level scope (union of all denials).")
                    .font(.system(size: 11))
