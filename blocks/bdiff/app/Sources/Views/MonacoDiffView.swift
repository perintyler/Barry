...
struct MonacoDiffView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // Load the bundled Monaco HTML from the app bundle (production) or
        // the SPM resource bundle (development `swift run`)
        let htmlURL: URL? = Bundle.main.url(forResource: "monaco-diff", withExtension: "html")
            ?? BDiffResources.bundle?.url(forResource: "monaco-diff", withExtension: "html")
        if let htmlURL {
    ...
    func updateNSView(_ webView: WKWebView, context: Context) {
        ...
        // Theme changes — defer until Monaco is ready to avoid JS errors
        if coordinator.currentIsDark != isDark {
            ...
            if coordinator.isReady {
                ...
                // flushPending will apply the theme once Monaco signals ready
                coordinator.pendingUpdate = update
            }
        }

        // Content changes — only push to the webview once Monaco is ready;
        // calling evaluateJavaScript before the editor initializes causes
        // repeated "t.toUrl is not an object" errors that spam the log.
        ...
        // Comment state — track always (so flushPending picks it up),
        // but only evaluate JS when Monaco is ready
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        func userContentController(_ controller: WKUserContentController, didReceive message: WKScriptMessage) {
            ...
            case "jsError":
                ...
                // Suppress known-harmless Monaco initialization noise that fires
                // before the editor is usable — these spam hundreds of identical
                // lines (toUrl resolution, cross-origin Script error).
                if !isReady && (msg.contains("toUrl") || msg == "Script error." || msg == "Script error") {
                    break
                }
