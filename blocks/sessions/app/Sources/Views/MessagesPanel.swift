...
struct MessagesPanel: View {
            ...
            LazyVStack(spacing: 0) {
                ForEach(state.segments) { rendered in
                    ...
                    case .toolRow(let msg):
                        toolCallRow(msg)
                            .id(rendered.id)
                            // Work done inside a subagent is nested, not the
                            // agent's own action — indent it so the two read
                            // differently.
                            .padding(.leading, msg.parentToolUseId != nil ? 16 : 0)
                            .opacity(msg.parentToolUseId != nil ? 0.75 : 1)
    ...
    /// Detect `/skill-name` or `/block:skill-name` at the start of a user message.
    ...
    private static func parseSkillInvocation(_ content: String) -> SkillInvocation? {
        ...
        // Match /word or /word:word (block:skill format), followed by optional args
