...
struct CreateTraitSheet: View {
    ...
    @State private var selectedBounds: Set<String> = []
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            ForEach(editor.allBounds, id: \.id) { bound in
                                checkRow(
                                    ...
                                    isOn: selectedBounds.contains(bound.name)
                                ) {
                                    if selectedBounds.contains(bound.name) {
                                        selectedBounds.remove(bound.name)
                                    } else {
                                        selectedBounds.insert(bound.name)
                                    }
                                }
                            }
                        }
    ...
    private func create() async {
        ...
        let ok = await editor.createTrait(
            ...
            access: access,
            boundNames: selectedBounds.sorted()
        )
        isSaving = false
        if ok {
            isPresented = false
