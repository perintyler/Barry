...
struct IdentityBoundsPanel: View {
    @Bindable var editor: IdentityEditor
    @State private var isCreatingBound = false
    @State private var editingBound: BoundRecord?

    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            // Assigning a named bound is what a file-based identity can't do:
            // it has no row to hold a bound_id, so it copies deny rules inline
            // instead. Authoring bounds is a different question — a bound is a
            ...
            if editor.isFileBased {
                Text("This identity keeps its deny rules inline in identity.yaml, so it can't be assigned a named bound.")
                    .font(AppFont.sans(size: 11))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                Text("Named bounds are a database concept; a directory copies the rules instead so it stays self-contained. You can still author them here — traits reference bounds by name.")
                ...
                boundList(selectable: false)
            } else {
                boundList(selectable: true)
                ToggleRow(title: "None", subtitle: "No named bound", isOn: editor.selectedBoundId == nil) {
                    editor.selectBound(nil)
                }
            }
            ...
            Button {
                isCreatingBound = true
            } label: {
                Label("New Bound", systemImage: "plus")
                    .font(AppFont.sans(size: 11))
            }
            .buttonStyle(.plain)
            .foregroundStyle(Palette.blue)
        }
        .sheet(isPresented: $isCreatingBound) {
            CreateBoundSheet(editor: editor, isPresented: $isCreatingBound)
        }
        .sheet(item: $editingBound) { bound in
            EditBoundSheet(editor: editor, bound: bound, editingBound: $editingBound)
        }
    }

    /// The bound list, with or without the assignment control.
    ///
    ...
    @ViewBuilder
    private func boundList(selectable: Bool) -> some View {
        if editor.allBounds.isEmpty {
            Text("No named bounds yet.")
                .font(AppFont.sans(size: 11))
                .foregroundStyle(.secondary)
        } else {
            ForEach(editor.allBounds, id: \.id) { bound in
                HStack(spacing: 6) {
                    if selectable {
                        ToggleRow(
                            title: bound.name,
                            subtitle: bound.description,
                            isOn: editor.selectedBoundId == bound.id
                        ) { editor.selectBound(bound.id) }
                    } else {
                        VStack(alignment: .leading, spacing: 1) {
                            Text(bound.name).font(AppFont.sans(size: 12))
                            if let description = bound.description, !description.isEmpty {
                                Text(description)
                                    .font(AppFont.sans(size: 10))
                                    .foregroundStyle(.tertiary)
                                    .fixedSize(horizontal: false, vertical: true)
