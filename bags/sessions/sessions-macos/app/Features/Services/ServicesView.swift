...
public struct ServicesView: View {
    ...
    public var body: some View {
        ...
        // F13: polling is started once at app launch (AppDelegate.setup) and
        // its cadence is driven by popover visibility via
        // `ServicesState.setVisible`, not by this view appearing — a
        // `.task { appState.start() }` here used to re-arm the poll timer
        // every time the Services tab came on screen, on top of the one
