...
struct BarryService: Identifiable {
    ...
    var pid: Int?
    /// False for run-once scripts, which exit on success and so are never
    /// "down". Defaulted so existing call sites keep the old behaviour.
    var expectedToStayUp: Bool = true

    var isSelf: Bool { id == "com.barry.services" }
    var isRunning: Bool { health == .running || health == .unhealthy }

    /// Whether this service is in a state worth surfacing. Scheduled agents
    /// and run-once scripts rest at "stopped" by design and never count —
    /// see `attentionLevel`.
    var needsAttention: Bool {
        if health == .unhealthy { return true }
        return health == .stopped && expectedToStayUp
    }
