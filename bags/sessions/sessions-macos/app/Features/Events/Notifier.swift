...
final class Notifier {
    ...
    /// bundle identifier, and folding the events app into this one changed the
    /// identifier — so a user who had already allowed BarryEvents starts over
    /// here, and "events stopped notifying me" is otherwise unfindable.
    private(set) var wasDenied = false

    /// Called when authorization state settles, so the UI can offer a way out.
    ...
    private func clickPayload(for event: BarryEvent, webBaseURL: URL) -> [String: Any] {
        if case .string("bag_auth") = event.data["action"] {
            ...
            if !bags.isEmpty {
                return ["action": "bag_auth", "bags": bags]
            }
        }
