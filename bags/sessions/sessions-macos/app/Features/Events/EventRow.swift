...
private struct CollapsibleText: ViewModifier {
    ...
    /// Line height comes from the font's own metrics rather than a hidden
    /// `Text(" ")` probe measured through a second `GeometryReader`. The probe
    /// measured a constant — same family, same size, every row — but paid for
    /// it with an extra laid-out view and a preference write per row, per
    /// layout pass, in a feed that lays out eagerly. Cached in `AppFont`.
    private var lineHeight: CGFloat { AppFont.sansLineHeight(size: 12) }
