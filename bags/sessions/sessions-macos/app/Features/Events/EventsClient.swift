...
struct EventsClient {
    ...
    /// Ask the API to begin OAuth for a bag. Best-effort: the browser tab is
    /// the real feedback, and the endpoint is idempotent while an attempt is
    /// already pending.
    func startBagAuth(_ bag: String) async {
        let url = baseURL.appendingPathComponent("api/v1/identities/bags/\(bag)/auth")
        try? await post(url)
    }
