...
class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setFeedVisible(popoverOpen: Bool) {
        ...
        // F13: the same popover-open/close transition also drives Services'
        // poll cadence — 5s while the popover is open, 30s while hidden,
        // rather than the old always-5s-forever. Funneled through this one
        // method rather than a call at each of the 4 popover-open/close
        // sites above, so a future call site can't forget it.
        servicesState.setVisible(popoverOpen)
    }
