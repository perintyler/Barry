...
struct InfoPanel: View {
    ...
    @State private var profileDefaults: [IdentityDefaults] = []
    ...
    var body: some View {
        ...
        .task {
            ...
            if let identities = try? await client.fetchIdentityDefaults() {
                profileDefaults = identities
            }
        }
    }
    ...
    private func commitReadOnly(_ readOnly: Bool) {
        Task {
            do {
                let bound: [String: Any] = readOnly
                    ? ["deniedAccess": ["write"]]
                    : ["deniedAccess": [String]()]
                try await client.updateBound(sessionId: session.id, bound: bound)
                onSessionUpdated()
            } catch {
                errorMessage = error.localizedDescription
            }
