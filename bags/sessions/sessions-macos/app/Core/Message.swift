...
public struct Message: Identifiable {
    ...
    /// Memberwise init for fixtures and tests — production code always goes
    /// through the `Decodable` conformance below, which is what normalizes
    /// `input`/`result` and computes `toolInputSummary`. This one does not,
    /// so a test building a `tool_start` fixture that needs
    /// `toolInputSummary` populated must set it explicitly.
    public init(
        type: String,
