...
/**
 * Default consecutive-failure breaker threshold (astra-review F17).
 *
 * Before this fix `runSweep`'s loop had no cap at all: a session whose
 * generation throws every tick (a malformed transcript, a DB hiccup specific
 * to that row) got retried by every single sweep tick forever — `failed`
 * just climbed with nothing checking it. ModelUnavailableError already
 * aborts the WHOLE sweep correctly (the model itself is down, not this one
 * session), so this breaker is deliberately scoped to ordinary per-session
 * errors, which are the case that had no guard.
 */
export const DEFAULT_MAX_CONSECUTIVE_FAILURES = 3;

/**
 * Whether the sweep loop should stop because ordinary (non-model-unavailable)
 * failures have piled up back-to-back.
 *
 * Pure so the threshold logic — not just "does 3 exist somewhere" but "does
 * the counter reset on a success or a skip, not just increment forever" — is
 * testable without constructing a fake session/DB harness.
 */
export function shouldBreakOnConsecutiveFailures(
  consecutiveFailures: number,
  max = DEFAULT_MAX_CONSECUTIVE_FAILURES,
): boolean {
