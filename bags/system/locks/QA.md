...
bag owns the storage, the path keying, and two read-only views.
...
```bash
cd ~/repos/barry/bags/locks && npx tsc --noEmit
```
...
```bash
cd ~/repos/barry/bags/locks && pnpm test 2>&1 | tail -5
```
...
```bash
cd ~/repos/barry/bags/locks && npx tsx -e "
import('/Users/tyler/repos/barry/bags/locks/src/tools.js').then(m => {
...
```bash
cd ~/repos/barry/bags/locks && cat > /tmp/locks-qa-run.ts <<'EOF'
import { acquireForEdit } from "/Users/tyler/repos/barry/bags/locks/src/acquire.js";
import { locksContention } from "/Users/tyler/repos/barry/bags/locks/src/tools.js";
...
cd ~/repos/barry/bags/locks && npx tsx -e "
import('/Users/tyler/repos/barry/bags/locks/src/acquire.js').then(({ acquireForEdit }) =>
...
```bash
cd ~/repos/barry/bags/locks && npx tsx -e "
import { parseManifest, getSkillsDirs } from '@barry-rocks/bags';
const m = parseManifest('/Users/tyler/repos/barry/bags/locks');
console.log(m.name, '| tools:', m.toolsEntry?.entry, '| deferred:', (m.toolsEntry?.deferred ?? []).join(','), '| skills:', getSkillsDirs('/Users/tyler/repos/barry/bags/locks').length);
" 2>&1 | grep -v DEP0205
```
...
```bash
test -f ~/repos/barry/bags/locks/skills/resolve-lock-conflict/SKILL.md && sed -n '2p' ~/repos/barry/bags/locks/skills/resolve-lock-conflict/SKILL.md
```
...
import('/Users/tyler/repos/barry/bags/locks/src/repo.js').then(({toRepoPath})=>{
...
import('/Users/tyler/repos/barry/cli/src/lib/settings.js').then(({ buildMergedSettings }) => {
  ...
  const ok = ['Write(*)','Edit(*)'].every(t => deny.includes(t));
...
```bash
cd ~/repos/barry/packages/agent-bound && npx tsx -e "
import('@barry-rocks/agent-bound').then(({ classifyFileWrite: f }) => {
  const bag = [\`python3 -c \"open('a.ts','w').write('x')\"\`, \"sed -i '' 's/a/b/' a.ts\", 'echo hi > a.ts', \`uv run python -c \"open('a.ts','w')\"\`];
  ...
  const bad = [...bag.filter(c => !f(c)), ...allow.filter(c => f(c))];
  console.log(bad.length === 0 ? 'OK guard correct on ' + (bag.length + allow.length) + ' cases' : 'BAD: ' + JSON.stringify(bad));
});
" 2>&1 | grep -v DEP0205
```

**Expected:** `OK guard correct on 9 cases`. The allow list matters more than the
bag list — a guard that fires on `pnpm test > /dev/null` gets switched off.
...
```bash
cd ~/repos/barry && npx tsx -e "
import('/Users/tyler/repos/barry/bags/locks/src/tools.js').then(async m => {
...
```bash
cd ~/repos/barry/packages/agent-bound && npx tsx -e "
import('@barry-rocks/agent-bound').then(({ isProgramDenied: d }) => {
...
### 15. Bag resolves through the CLI

```bash
barry bag show locks 2>&1 | head -14
```
...
- [ ] `tsc --noEmit` is clean
- [ ] Unit tests pass (bag + `wrapEditIntent` suite)
...
- [ ] Native `Write`/`Edit` are denied for every CLI session
