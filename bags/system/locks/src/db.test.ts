...
import { getLock, getWaiters, contendedPaths, initSchema, reapStaleLocks, recordEvent, type LockDb } from "./db.js";
...
describe("contendedPaths", () => {
  it("finds a held path with a waiter, without the caller knowing the path in advance", () => {
    insertLock({ sessionId: "holder" });
    db.prepare(
      `INSERT INTO intents (repo_root, rel_path, session_id, intent, state) VALUES ('/repo','a.ts','waiter','fix bug','waiting')`,
    ).run();

    const found = contendedPaths(db);
    expect(found).toHaveLength(1);
    expect(found[0]).toMatchObject({ repoRoot: "/repo", relPath: "a.ts" });
    expect(found[0].holder?.session_id).toBe("holder");
    expect(found[0].waiters.map((w) => w.session_id)).toEqual(["waiter"]);
  });

  it("omits a path with no waiters, even if it is held", () => {
    insertLock({ sessionId: "holder" });
    expect(contendedPaths(db)).toHaveLength(0);
  });

  it("reports holder undefined for a path everyone is waiting on but no one holds", () => {
    // The lock expired/was released, but the waiter's intent row survives —
    // exactly the free-now case locks_contention flags via holder=null.
    for (const session of ["w1", "w2"]) {
      db.prepare(
        `INSERT INTO intents (repo_root, rel_path, session_id, intent, state) VALUES ('/repo','a.ts',?,?,'waiting')`,
      ).run(session, `${session} work`);
    ...
    const found = contendedPaths(db);
    expect(found).toHaveLength(1);
    expect(found[0].holder).toBeUndefined();
    expect(found[0].waiters).toHaveLength(2);
  });

  it("filters to one repo when repoRoot is given", () => {
    insertLock({ repoRoot: "/repo-one", sessionId: "holder" });
    db.prepare(
      `INSERT INTO intents (repo_root, rel_path, session_id, intent, state) VALUES ('/repo-one','a.ts','waiter','x','waiting')`,
    ).run();
    insertLock({ repoRoot: "/repo-two", sessionId: "holder2" });
    db.prepare(
      `INSERT INTO intents (repo_root, rel_path, session_id, intent, state) VALUES ('/repo-two','a.ts','waiter2','y','waiting')`,
    ).run();

    const found = contendedPaths(db, "/repo-one");
    expect(found).toHaveLength(1);
    expect(found[0].repoRoot).toBe("/repo-one");
  });

  it("returns no rows when nothing is contended", () => {
    expect(contendedPaths(db)).toEqual([]);
  });
});
