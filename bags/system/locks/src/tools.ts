...
import { z } from "zod";
import { getDb, getLock, contendedPaths, type EventRow, type IntentRow } from "./db.js";
...
export const locksContention = defineTool({
  ...
  handler: async ({ repo_root }, context) => {
    ...
    const contended = contendedPaths(db, repo_root).map((entry) => ({
      path: formatRepoPath({ repoRoot: entry.repoRoot, relPath: entry.relPath }),
      holder: entry.holder
        ? {
            session: short(entry.holder.session_id),
            sessionId: entry.holder.session_id,
            mine: entry.holder.session_id === callerId,
            intent: entry.holder.intent,
            age: ageOf(entry.holder.acquired_at),
          }
        : null,
      waiting: entry.waiters.map((waiter) => ({
