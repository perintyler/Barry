...
/**
 * `acquireForEdit` calls `findDeadSessions` on every edit-lock acquisition —
 * one of these round trips per hook-triggered Edit/Write, each fetching a
 * fully-enriched up-to-100-session listing (message activity, progress,
 * camelCase serialization for every field) just to read `.id`. `fields=id`
 * asks the route to skip enrichment; this cache then lets a burst of
 * near-simultaneous edits share one probe. The TTL is short relative to
 * PROBE_TIMEOUT_MS so a genuinely dead session is not reported live for long,
 * and it still fails open exactly as before — a cached `null` is `null`.
 */
const LIVENESS_CACHE_TTL_MS = 2000;
let cached: { at: number; value: Promise<Set<string> | null> } | null = null;

/** Test hook: clear the liveness probe cache. */
...
async function fetchActiveSessionIds(): Promise<Set<string> | null> {
  ...
  try {
    ...
    // sessions are dead" — which would free live locks. fields=id skips the
    // route's message/progress enrichment, which this caller never reads.
    const resp = await fetch(`${apiBaseUrl()}/sessions?active=true&limit=100&fields=id`, {
      headers,
    ...
    // The `?active=true` query above is what filters to live sessions — it
    // matches on the DB `active` column server-side. The serialized session does
    // not carry that column, so there is nothing further to filter on here.
...
export async function getActiveSessionIds(): Promise<Set<string> | null> {
  const now = Date.now();
  if (cached && now - cached.at < LIVENESS_CACHE_TTL_MS) return cached.value;
  const value = fetchActiveSessionIds();
  cached = { at: now, value };
  return value;
}
