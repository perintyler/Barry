...
(the bdiff bag) are scoped to branch/commit contexts, so there's no way to
...
- **Live loop**: the running agent is nudged/polls via the bdiff bag
...
- bdiff review service + bag (`servers/bdiff`, `bags/bdiff`); comments table
...
- Comment → session push channel fancier than bag polling (e.g. prompt-queue
