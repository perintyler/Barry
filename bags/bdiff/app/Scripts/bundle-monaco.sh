...
// Suppress known-harmless Monaco initialization noise (e.g. 't.toUrl' during
// async service startup, 'Script error' from cross-origin frames) — these
// fire before the editor is usable and spam the log with hundreds of
// identical lines.
var _monacoInitDone = false;
window.onerror = function(message, source, lineno, colno) {
  var msg = String(message);
  if (!_monacoInitDone && (msg.indexOf('toUrl') !== -1 || msg === 'Script error.' || msg === 'Script error')) return;
  if (window.webkit?.messageHandlers?.bridge) {
    window.webkit.messageHandlers.bridge.postMessage({
      type: 'jsError', message: msg, source: String(source || ''), line: lineno || 0,
    });
  }
};
window.addEventListener('unhandledrejection', function(e) {
  var msg = String(e.reason);
  if (!_monacoInitDone && (msg.indexOf('toUrl') !== -1 || msg.indexOf('Script error') !== -1)) return;
  if (window.webkit?.messageHandlers?.bridge) {
    window.webkit.messageHandlers.bridge.postMessage({
      type: 'jsError', message: 'unhandledrejection: ' + msg, source: '', line: 0,
    });
  }
});
...
// Signal readiness after a short delay to let Monaco's async service layer
// finish initializing. The t.toUrl errors come from the ITextResourceConfigurationService
// not being registered yet when createDiffEditor triggers internal URI resolution.
// requestAnimationFrame ensures layout is stable; the setTimeout gives the service
// registration microtasks time to complete.
...
#
# Create the directory first: monaco-diff.html is gitignored and is the only
# thing that ever lived here, so the directory does not survive a clone (or a
# `git mv` of the app) and the copy fails on an otherwise-successful build.
mkdir -p "$BDIFF_DIR/Sources/Resources"
