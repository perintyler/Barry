 ...
 * Postgres tables everything uses today (via @barry/session-client, whose
...
app.post("/sessions/:id/messages/batch", async (req, res) => {
  ...
  const sessionId = req.params.id;
  const { epoch, providerSessionId, messages } = parsed.data;
  ...
  try {
    for (const { sequence, message } of messages) {
      await persistWsMessage(sessionId, message as Parameters<typeof persistWsMessage>[1], sequence, providerSessionId);
    }
  } catch (err) {
