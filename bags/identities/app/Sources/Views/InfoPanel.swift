    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                ...
                // Only worth a row when it says something the Name row doesn't.
                if identity.label != identity.name {
                    infoField("Display name") {
                        Text(identity.label)
                            .font(.system(size: 13))
                    }
                }
                ...
                infoField("Source") {
                    HStack(spacing: 5) {
                        Image(systemName: identity.isFileBased ? "folder" : "cylinder.split.1x2")
                            .font(.system(size: 10))
                            .foregroundStyle(.tertiary)
                        Text(identity.isFileBased ? "~/.identity/identities/\(identity.name)/" : "Database")
                            .font(.system(size: 11, design: identity.isFileBased ? .monospaced : .default))
                            .foregroundStyle(.secondary)
                            .textSelection(.enabled)
                    }
                }

                // A file-based Identity's token is derived from its name rather
                // than issued, so showing it would invite copying a value that
                    ...
                    infoField("Token") {
                        HStack(spacing: 6) {
                            ...
                            Button("Copy") {
                                ...
                                NSPasteboard.general.setString(identity.token, forType: .string)
                            }
                            .buttonStyle(.plain)
                            .font(.system(size: 10))
                ...
                if let created = identity.createdAt {
                    infoField("Created") {
                ...
                infoField("Last Used") {
                    Text(identity.displayLastUsed)
                        .font(.system(size: 12))
                        .foregroundStyle(.secondary)
                }

                if let email = identity.vaultEmail {
                    infoField("Vault") {
                        HStack(spacing: 5) {
                            Circle()
                                .fill(Color.green)
                ...
                if !identity.envKeys.isEmpty {
                    ...
                    FlowLayout(spacing: 6) {
                        ForEach(identity.envKeys, id: \.self) { key in
                            Text(key)
                ...
                // The save succeeded — this reports a value the server accepted
                // but couldn't vouch for, so it reads as a caution, not a failure.
                if let warning = editor.warningMessage {
                    Text(warning)
                        .font(.system(size: 11))
