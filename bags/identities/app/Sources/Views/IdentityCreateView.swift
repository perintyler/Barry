...
struct CreateIdentityView: View {
    ...
    private var bagsSummary: String {
        "\(selectedBags.count) selected"
    }

    @ViewBuilder
    private var bagsContent: some View {
        ForEach(allBags) { bag in
            Button {
                if selectedBags.contains(bag.name) {
                    selectedBags.remove(bag.name)
                } else {
                    selectedBags.insert(bag.name)
                }
            } label: {
                HStack(spacing: 10) {
                    checkMark(isChecked: selectedBags.contains(bag.name))
                    VStack(alignment: .leading, spacing: 1) {
                        Text(bag.name)
                            .font(.system(size: 13, weight: .medium))
                            .foregroundStyle(.primary)
                        if let desc = bag.description {
                            Text(desc)
                                .font(.system(size: 11))
