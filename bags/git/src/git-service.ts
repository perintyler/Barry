 ...
 */
export function isTimeoutError(error: { killed?: boolean; signal?: string | null; code?: string | number }): boolean {
...
async function runGit(
  ...
  try {
    ...
    if (args[0] === "diff" && execError.code === 1 && execError.stdout !== undefined) {
...
export class GitService {
  async getStatus(repoPath: string): Promise<GitStatus> {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      const source = /[RC]/.test(x + y) ? records[++i] : undefined;
      ...
      if (x === "U" || y === "U" || (x === "A" && y === "A") || (x === "D" && y === "D")) {
        ...
        // Both ends are needed to commit a rename rather than just add its destination.
        if (source && x === "R") staged.push(source);
        if (source && y === "R") unstaged.push(source);
      }
    }
  ...
  async commit(repoPath: string, message: string, options: { all?: boolean; files?: string[]; env?: Record<string, string> } = {}): Promise<string> {
    const cleanedMessage = stripCoAuthors(message);
    const args = ["--literal-pathspecs", "commit"];
    if (options.files) {
      const status = await this.getStatus(repoPath);
      const { stdout: gitDir } = await runGit(repoPath, ["rev-parse", "--absolute-git-dir"]);
      if (status.conflicts.length || ["MERGE_HEAD", "CHERRY_PICK_HEAD", "REVERT_HEAD", "rebase-merge", "rebase-apply", "sequencer", "BISECT_START"].some((state) => fs.existsSync(path.join(gitDir.trim(), state)))) {
        throw new Error("Scoped commits are refused during unresolved conflicts or an in-progress Git operation. Finish or abort that operation directly with Git first.");
      }
      if (options.files.length === 0) return "Nothing to commit for this session.";
      const { stdout: hooks } = await runGit(repoPath, ["rev-parse", "--path-format=absolute", "--git-path", "hooks"]);
      if (["pre-commit", "prepare-commit-msg", "commit-msg", "post-commit"].some((hook) => fs.existsSync(path.join(hooks.trim(), hook)))) {
        throw new Error("Path-limited commits with commit hooks are not supported safely. Review and commit directly with Git in an isolated worktree; hooks were not bypassed.");
      }
      // --only reads the worktree, so staged-only selections must still be fully staged.
      // Concurrent same-path edits after this check are unsupported; use an isolated worktree.
      if (!options.all && options.files.some((file) => !status.staged.includes(file) || status.unstaged.includes(file) || status.untracked.includes(file))) {
        throw new Error("Selected paths are partially staged or have ambiguous worktree changes. Stage the whole selected files, use Git directly for staged hunks, or explicitly use all: true for whole-file commits.");
      }
      const { stdout: untracked } = await runGit(repoPath, ["--literal-pathspecs", "ls-files", "--others", "--exclude-standard", "-z", "--", ...options.files]);
      const newFiles = untracked.split("\0").filter(Boolean);
      if (newFiles.length) await this.add(repoPath, { files: newFiles });
      args.push("--only");
    } else if (options.all) {
      args.push("-a");
    }
