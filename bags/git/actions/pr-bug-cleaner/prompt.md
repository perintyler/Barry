...
**Important**: All scripts must be run from the repository root directory (where `.git` is located), not from the skill directory. Use the full path to the script via `${ACTION_DIR}`.
...
```bash
uv run ${ACTION_DIR}/scripts/fetch_pr_checks.py [--pr NUMBER]
```
...
```bash
uv run ${ACTION_DIR}/scripts/fetch_pr_feedback.py [--pr NUMBER]
```
...
Run `${ACTION_DIR}/scripts/fetch_pr_feedback.py` to get categorized feedback already posted on the PR.
...
Run `${ACTION_DIR}/scripts/fetch_pr_checks.py` to get structured failure data.
...
Stage with the `add` tool, then commit and push with the `barry` CLI:
...
**Important**: Use `barry commit` / `barry push`, not raw `git commit` / `git push`. Raw `git` is blocked in this session, and `barry commit` also ensures commits are authored by Barry's GitHub user. Stage files with the `add` tool (or `barry commit -a` to stage all tracked changes and commit in one step).

### 7. Monitor CI and Address Feedback

Poll CI status and review feedback in a loop instead of blocking:

1. Run `uv run ${ACTION_DIR}/scripts/fetch_pr_checks.py` to get current CI status
...
4. If checks are still pending:
   a. Run `uv run ${ACTION_DIR}/scripts/fetch_pr_feedback.py` for new review feedback
...
5. After all checks pass, do a final feedback check: `sleep 10`, then run `uv run ${ACTION_DIR}/scripts/fetch_pr_feedback.py`. Address any new high/medium feedback — if changes are needed, return to step 6.
