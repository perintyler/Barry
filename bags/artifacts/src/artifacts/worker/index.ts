...
/**
 * Artifacts Cloudflare Worker (part of @barry-rocks/artifacts-bag)
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if ((uuidHit || legacyHit) && response.status === 200) {
      ...
      const previewId = uuidHit ? uuidHit[1] : legacyHit ? legacyHit[2] : '';
...
async function handleRequest(
  ...
  if (uuidMatch && (request.method === 'GET' || request.method === 'POST')) {
    ...
    const { namespace }: { namespace: string } = await resolveRes.json();
  ...
  // Everything below this line reads or writes artifacts. Everything above it
  // is a preview or raw-file route, which is deliberately public: those URLs
  // are the shareable links, and requiring a secret would break every one.
  //
  // The API and admin routes had no such excuse. X-Artifacts-Namespace is
  // supplied by the caller, so while this worker answered on a public
  // hostname, `curl /admin/namespaces` returned the full namespace list and
  // any namespace could be read or written by anyone who named it. The CORS
  // block above has always advertised an Authorization header; nothing read
  // it until now.
  //
  // Optional by design: ARTIFACTS_AUTH_TOKEN unset leaves the worker open,
  // because the SDK is deployed outside Barry too and this must not break
  // those. That makes it the operator's job to set it — Barry's deployment
  // does, and `barry bag deploy artifacts` diffs it as a declared secret.
  if (env.ARTIFACTS_AUTH_TOKEN) {
    const offered = request.headers.get('Authorization');
    if (offered !== env.ARTIFACTS_AUTH_TOKEN) {
...
async function resolveNamespace(env: Env, url: URL, previewId: string): Promise<string | null> {
  ...
  const { namespace }: { namespace: string } = await res.json();
  return namespace;
}
...
async function isPasswordProtected(env: Env, url: URL, namespace: string, previewId: string): Promise<boolean> {
  ...
  const previews: { id: string; passwordProtected?: boolean }[] = await res.json();
...
async function screenshotPreview(env: Env, previewUrl: string): Promise<Response> {
  const browser = await puppeteer.launch(env.BROWSER as BrowserWorker);
  try {
    const page = await browser.newPage();
    await page.setViewport({ width: 1200, height: 630 });
    await page.goto(previewUrl, { waitUntil: 'networkidle0', timeout: 10000 });
    const screenshot = await page.screenshot({ type: 'png' });
    return new Response(screenshot as Uint8Array<ArrayBuffer>, {
      headers: {
