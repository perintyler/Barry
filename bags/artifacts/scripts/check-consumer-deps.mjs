...
/**
 * Verify that a consumer of @barry-rocks/artifacts-bag declares every dependency
 * the SDK source needs, at the same version.
 *
 * Why this exists
 * ---------------
 * Consumers like ~/vantage/artifacts do not `npm install` this package. They
 * alias the bare specifier straight at `bags/artifacts/src/artifacts` (see their
 ...
 *
 *   3. A consumer on a different MAJOR of the types packages typechecks our
 *      source against a different platform API than we do. This one was
 *      invisible for months: `required` below read `dependencies` only, while
 *      @cloudflare/workers-types is a devDependency, so the drift was never
 *      looked at rather than merely tolerated.
 *
 * Usage:
...
/**
 * Type-only packages the consumer compiles our source against. They are
 * devDependencies here, so they are NOT in `required` above — that omission is
 * why @cloudflare/workers-types v4-vs-v5 drift went unreported for months.
 *
 * Checked on MAJOR only: a consumer a few minor releases behind on a types
 * package is normal, but a major is a different platform API surface, and our
 * source is what fails to compile in their build.
 */
const TYPES_CRITICAL = new Set(['@cloudflare/workers-types']);

const requiredTypes = Object.entries(sdkPkg.devDependencies || {}).filter(
  ([name]) => TYPES_CRITICAL.has(name),
);
...
try {
  ...
  // The consumer checkout lives outside this repo, so CI machines do not have
  // it. Absence must be DECLARED rather than inferred: silently passing when
  // the file is missing is what let this guard sit inert, and silently failing
  // would make every CI run red for a file it can never have.
  if (err.code === 'ENOENT' && process.env.BARRY_ALLOW_MISSING_CONSUMER === '1') {
    console.log(`- consumer checkout absent at ${resolved}; skipped (BARRY_ALLOW_MISSING_CONSUMER=1)`);
    process.exit(0);
  }
  console.error(`Cannot read consumer package.json at ${resolved}: ${err.message}`);
  if (err.code === 'ENOENT') {
    console.error('  If this machine is not expected to have the consumer checkout,');
    console.error('  set BARRY_ALLOW_MISSING_CONSUMER=1 to declare that explicitly.');
  }
  process.exit(2);
}
...
// A types package the consumer does not declare at all is not drift: they may
// simply not typecheck our source. Only a declared, mismatched major counts.
const major = (range) => Number(baseVersion(range).split('.')[0]);
for (const [name, version] of requiredTypes) {
  if (!(name in have)) continue;
  if (major(have[name]) !== major(version)) typesDrift.push([name, have[name], version]);
}
...
if (!missing.length && !criticalDrift.length && !typesDrift.length) {
  console.log(`✓ ${consumerPkg.name} matches ${sdkPkg.name} on rendering and platform-types dependencies`);
  process.exit(0);
}

console.error(`✗ ${consumerPkg.name} is out of sync with ${sdkPkg.name}\n`);
...
if (typesDrift.length) {
  console.error('  Platform types drift — the consumer compiles our source against a');
  console.error('  different major of these, so our code can fail to build in their tree:');
  for (const [name, theirs, ours] of typesDrift) {
    console.error(`    ${name}: consumer has ${theirs}, SDK declares ${ours}`);
  }
  console.error('');
}
...
const fix = [
  ...missing.map(([n, v]) => `${n}@${v}`),
  ...criticalDrift.map(([n, , v]) => `${n}@${v}`),
  ...typesDrift.map(([n, , v]) => `${n}@${v}`),
];
