...
export const remindersAdd = defineTool({
  ...
  handler: async ({ message, when, channel }, context) => {
    ...
    if (!resolved) {
      throw new Error(
        ...
        "(e.g. via `barry vault set-env <barry> BARRY_REMINDER_CHANNEL slack`).",
      );
    }
  ...
  cliFormat: (result) =>
    `Reminder ${result.id} set for ${new Date(result.dueAt).toLocaleString()} via ${result.channel}\n  ${result.message}`,
});
...
export const remindersList = defineTool({
  ...
  cliFormat: (result) => {
    if (result.length === 0) return "No reminders.";
    return result.map(formatReminder).join("\n");
  },
});
...
export const remindersCancel = defineTool({
  ...
  cliFormat: (result) => `Cancelled ${result.id}: ${result.message}`,
});
