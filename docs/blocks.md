...
A block is Barry's unit of agent capability. It keeps a tool or integration
together with the skills that explain how to use it, the traits that control
access to it, and any credentials it requires.

An identity selects blocks. At session start Barry merges their capabilities and
exposes only what the session's traits and scope allow.

## Local and remote blocks

A local block is a directory with a versioned `block.yaml` manifest. Local
tool entrypoints are loaded by Barry's unified MCP server; they are not separate
long-running services. A local block may instead declare its own MCP server or
reference other MCP servers.

A remote block is a URL or command for a third-party MCP server. Barry connects
to it through the unified MCP server, discovers its tools, and applies the same
trait filtering used for local tools.

## Which blocks ship with Barry

Barry ships a small set of **builtin** blocks. Everything else lives in its own
repo and is registered per machine.

The line is whether a block is useful to someone who has just installed Barry and
configured nothing:
...
blocks; they simply live elsewhere.
...
A `packages/*` entry whose **only** consumer is one block is block internals in the
wrong place — move it into the block. A package with genuinely independent
consumers stays shared. When two *blocks* need the same library, the owning block
exports it as a subpath rather than a third package existing to serve both.

## How local blocks are built

Local block tools are **bundled to plain JavaScript before they run**. Barry does
not import a block's TypeScript at runtime.
...
block could work on your machine and silently vanish in production. Bundling
removes the whole class of problem: any valid TypeScript is fine, and an
npm-installed block works.

Build output lives in `~/Library/Caches/Barry/blocks/<name>/`, separate from the
registry at `~/Library/Application Support/Barry/blocks.yaml`. The split is
...
```bash
barry block build            # build every local block
barry block build git -w     # rebuild one block on change
```
...
0.2s. Set `BARRY_BLOCKS_BUILT=0` to fall back to loading raw TypeScript.
...
`version` is the block's own release and is optional — there is only one
manifest format, so nothing branches on it.

A block can then add only the capabilities it needs:
...
- **Do not repeat the namespace in a tool name.** The prefix already carries it;
  ...
  it `status`.
- **Bare names need only be unique within a namespace.** Ten blocks define
...
Blocks that shell out to a CLI or launch an MCP server through a runner
...
Each entry is checked on PATH. `block show` renders ✓/✗ per binary,
`barry add-block` warns (non-blocking) with the install command, and the MCP
server fails fast at connect time — logging `block_proxy.missing_dependency`
with the install hint instead of timing out silently and dropping the block's
...
Block connections are pooled by `blockName:credentialHash` and shared across the
...
The block then joins the deferred set (no eager shared connection) and each
...
pay for processes and get no separation. Those blocks are deliberately *not*
session-scoped.
...
Every block gets up to two traits automatically:

- `<block-name>` grants its available namespaces with read-write access, plus
  the block's skills
- `<block-name>-read` grants the namespaces known to be readable (plus skills)

A skills-only block (no tool namespaces — e.g. `qa`) still gets its
`<block-name>` trait so sessions can opt into the skills. Custom traits can
expose a smaller, task-oriented subset.

## Trait-gated skills

A trait carries a `skills` list (the skill directory names it grants). Giving a
session that trait mounts those skills at spawn — independently of which blocks
the identity enables. This lets a session pull in a block's skills without
enabling the whole block on the identity.

```bash
barry block sync-traits qa        # register the trait (with its skills) in the DB
barry start --traits qa          # CLI session with the block's skills mounted
...
(`resolveSkillDirs` → `buildSkillsPlugin` in `@barry/blocks`). The plugin writes
...
mount into). Block tools, by contrast, work across all providers via MCP trait
filtering.

## Jobs and services

A block can declare background work. `scripts/launchd/setup` installs it as
...
jobs:
  nightly-sync:
    ...
    command: bash
    args: ["scripts/nightly-sync"]   # relative paths resolve inside the block
    working-directory: "."
...
Ports are checked for collisions against core services and other blocks before
...
service's plist. `barry block doctor` reports conflicts too, globally: two
blocks can collide while no single identity enables both.

Labels are `com.barry.block.job.<block>.<job>` and `com.barry.block.<block>.<service>`.
Logs land in `~/.barry/logs/`.

**Paths must stay inside the block.** A relative `args`/`working-directory` path
reads as block-local, so anything resolving outside the block directory is
...
no session, so `block access <name> disabled` — or removing the block — retracts
its agents, while `barry add-block`/`remove-block`, which control barry
membership, do not affect them. Setup prunes any agent nothing declares anymore, so
uninstalling a block stops its background work.

Barry does not install a block's npm dependencies. `block add`,
`barry add-block` and `block show` warn when a block declares dependencies it hasn't installed — run
`pnpm install` in the block. Declare host binaries under `dependencies:`.
...
same barry, traits, blocks and skills, then runs a single prompt to completion
and exits:
...
`--traits` is what mounts a block's skills, independently of which blocks the
...
`enabled: false`, so installing the block does not start it. Flipping it back to
...
A block can ship a GUI application. An app is **not** a service: it is built,
...
apps:
  MyApp:
    ...
    build:
      ...
      output: app/.build/MyApp.app   # block-relative; never absolute
...
block's business — produced by its own build command. This is what keeps one
...
this block's build artifact, and adopting an arbitrary `.app` that then runs as
a login item is a trust escalation. A build that exits zero but leaves no valid
bundle is a failure, not a skip.

Apps are built by `barry block build --apps` (and by `./install`), never by
`scripts/launchd/setup` — setup installs whatever bundle already exists, because
it runs on every `block enable` and a cold `swift build` is slow. Remote blocks
are skipped entirely: running a build command from a block fetched over the
network would be arbitrary code execution at install time.

## The resource registry

Services and apps are published to
`~/Library/Application Support/Barry/block-resources.json`, keyed
`<block>.<name>`:

```json
{"version": 1, "resources": {
  "bdiff.review": {
    "block": "bdiff", "name": "review", "kind": "service",
    "label": "com.barry.block.bdiff.review",
    "url": "http://127.0.0.1:3862", "health": "/health"
  }
}}
```

A block's own tools resolve their service this way — `getBlockResourceUrl("bdiff",
"review")` — instead of reaching for a central port table, which is what lets a
block own a server without core knowing the block exists.
...
`block-resource` secret source alongside keychain and vault — useful when a
...
The block ships two deferred read-only tools, `locks_contention` and
...
Every path fails **open**: a missing block, an unreadable DB, or a command the
tokenizer cannot read lets the edit through. Only a confirmed conflict stops one.

## Verbs & nouns

A block can describe *what it does* in terms the agent reasons in, not just the
...
- A **verb** is an action — `debug`, `notify`, `compose`. It carries `synonyms`
  ...
  approach* that action with this block.
- A **noun** is a domain object the agent acts on — `virtual-tags`, `logs`,
  `clip`. It carries a `description` (the block's domain knowledge), the tools
...
interface, just structured context. At session start the active blocks' verbs and
nouns are compiled into a single *capability map* and appended to the system
prompt, through the same path as block `instructions`. Because the map spans every
active block, the agent can **compose a verb from one block with a noun from
...
`vantage-core`) — without either block knowing about the other. Two blocks
...
above); verbs and nouns are pure prompt context. A block that declares neither
adds nothing to the prompt.

Source: `compileCapabilityMap` in `@barry/blocks` (`capability-map.ts`), collected
by `mergePacks` and injected in `servers/api/src/sdk-manager.ts`.

## Create and use a local block

```bash
barry block create my-tools --path ~/repos/my-tools
# edit ~/repos/my-tools/block.yaml and add skills or tools
barry block add my-tools ~/repos/my-tools
barry add-block my-tools
barry block show my-tools
```

The registry is machine-local. Built-ins come from
`builtins/blocks.yaml`; user registrations live in
`~/Library/Application Support/Barry/blocks.yaml`, falling back to the older
`~/.barry/blocks.yaml` when that file is absent, and `BARRY_BLOCKS_CONFIG`
overrides both. An identity stores only the names of the blocks it enables.

Which store a command writes is what its name tells you. `block add`/`remove`
and `block access` write the registry; `barry add-block`/`remove-block` write a
identity row, and name the identity rather than defaulting to the current one.
`block enable`/`block disable` remain as aliases for the latter two.

## Add a remote block

```bash
barry block add notion https://mcp.notion.com/mcp
barry add-block notion
```

Remote blocks that use API keys declare environment variables in their registry
entry. Put those values in the identity or service environment that starts the
session; see [Environment](environment.md).

## Writing a third-party block

A block you distribute to other people is best shipped as a **remote block**: an
...
written for Barry, so defaulting it on would make every remote block pay a live
MCP round trip on load for nothing.

Measured against the remote blocks registered on a real machine at the time
(vault, notion, figma, playwright-remote, mobile-mcp, vantage-docs —
`playwright-remote` has since been retired in favour of the `browser` block):
**none** exposed any `barry://` resource, and probing them cost ~8s of serial
connection time. Turn it on for blocks you know serve the resources — which, in
practice, means blocks built for Barry.

The safety work is in place either way: discovery is bounded by a 5s timeout
(`BARRY_BLOCK_DISCOVERY_TIMEOUT_MS`), failures fall back to tools-only with a
warning rather than silently, and skill resolution no longer reloads every block
...
- **`command`-based entries in `barry://config/mcp-servers`** are dropped. A
  remote block must not be able to name a subprocess to run on someone else's
  machine; launching a local process stays an explicit choice in the user's own
  registry.
- **Trait `skills`** are filtered to skills your block actually published, so one
  block cannot grant another block's skills.

If your manifest fails validation, Barry does not drop the block — it falls back
to reading just `name`, `description` and `tools`, and logs why the rest was
ignored. Check `barry block show <name>`.

Note that a block's `instructions` are injected into the session system prompt.
Enabling a third-party block means trusting its instructions as much as its code.

### Reference implementation

Any local block can be served over MCP, which is the easiest way to see the wire
format and to test the remote path against something known-good:

```bash
barry block serve git --port 9878
barry block add git-remote http://127.0.0.1:9878/mcp   # add `resources: true`
barry block show git-remote                            # compare with `barry block show git`
```

## OAuth blocks

OAuth blocks such as Notion and Sentry do not open a browser during background
startup. Without cached authorization, Barry marks the block as needing auth and
leaves its tools unavailable.

Authorize deliberately with:

```bash
barry block auth <name>
```

An active agent can call the `block_auth` tool for the same flow. For OAuth blocks,
Barry opens one browser tab, waits for approval, and reconnects the block. Cached
tokens live under `~/.mcp-auth/` and refresh non-interactively when possible.

For CLI-delegated blocks (those with `auth:` in their manifest), `block_auth`
spawns the vendor CLI's login command (which opens the browser itself), polls the
`check` command until it exits 0, and optionally retries failed block connections.

## CLI bridge

Enabling a block exposes its tools as CLI commands. No static command
registration needed — the bridge reads block manifests at startup and creates
subcommands dynamically.

```
barry <block> <tool> [args]
...
Phase 2 imports the tools module on demand when a block subcommand is invoked.
`barry --help` lists block groups without importing any tool code.

### Conflict with static commands

Static Commander.js commands always win over block groups with the same name.
The escape hatch:

```bash
barry run <block> <tool> [args]     # always routes to the block tool
```

### Choosing which blocks get CLI commands

Create `~/.barry/cli.yaml` with the block names you want:
...
If the file doesn't exist, no blocks are exposed. Every block is still reachable
via `barry run <block> <tool>` regardless of config.

### Manifest options

A block can set `cli.alias` to override its CLI command name:
...
Completes block group names, tool subcommands, and `--flags` within a tool.
...
- `packages/blocks/` — registry, manifest, merge, and discovery rules
- `packages/tools/src/cli-adapter.ts` — Zod→CLI flag mapping and positional inference
- `cli/src/block-cli.ts` — CLI bridge: registration, dispatch, secret resolution, help
- `cli/src/commands/completion.ts` — shell tab completion
- `servers/mcp/` — local tool loading, remote proxying, and access filtering
- `blocks/` — built-in local blocks
- `builtins/blocks.yaml` — built-in registrations

Use `barry block list` and `barry block show <name>` for the current machine's
effective configuration; avoid maintaining a second block inventory in docs.
