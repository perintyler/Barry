...
The standard dev ports come from `packages/env`; important defaults are API
3854 and Postgres 15432. barry.works is a bag outside this repo
(`~/repos/bags/barry.works`), so its port lives in its own manifest. Use
`barry config` instead of relying on a copied port table when diagnosing the
current environment.
...
Production is still source-first for most services. The deliberate built
runtime contract in this repo is the plain-Node MCP bundle (barry.works, now an
external bag, builds its own assets). See
[Installation](installation.md#runtime-contract).

## Shared infrastructure

OrbStack runs Postgres for both environments:

- Postgres on host port 15432 (loopback only, TLS required), containing `barry_dev` and `barry`

This container is the reason a container runtime is required to install Barry at
all. As of 0.0.2 the config tables no longer live in it: bags, traits, bounds,
settings and repos moved to `~/.barry/config.db`, joining the seven subsystems
already on embedded SQLite. Sessions and messages stay here, so `barry start`
still needs this container. [Data stores](datastore.md) has the measurements and
what is left.

Barry Vault is no longer a container. It runs as a LaunchDaemon under its own
`_barryvault` account, reachable at
`unix:///usr/local/var/barry-vault/run/vault.sock`. See `bags/vault/CUTOVER.md`
for the migration and `bags/vault/README.md` for the isolation it provides.
...
machine. Barry's public-mirror release process is separate; it lives with the
`release` action in the [devops bag](https://github.com/perintyler/devops-bag).
...
`scripts/launchd/` for host networking and [launchd](launchd.md) for service
operation.
...
- The allowed identity is one email in
  `cloudflare_zero_trust_access_policy.barry_owner` (the cloudflare bags `infra/access.tf`,
  value in a gitignored tfvars file). An address that isn't on the policy gets
  **no email at all**, silence is the expected response to an unknown address,
  not a bug.
- The `*.barry.works` Access application covers every barry.works subdomain.
  The vault is **not** among them: it lives at `vault.barry.rocks`, a separate
  zone, behind its own Access application that reuses the same policies.
- Sign-in methods are Terraform-managed: `allowed_idps` pins One-Time PIN in
  the cloudflare bags `infra/access.tf`. **Adding a login method (Google, GitHub SSO) in
