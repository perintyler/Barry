...
/**
 * Start a stdio MCP server for a local pack.
 *
 ...
 *
 * stdout is reserved for MCP protocol — pack code must not write to it.
 */
...
export interface StartPackServerOptions {
  name: string;
  version?: string;
  tools: AnyToolDefinition[];
}

export async function startPackServer(options: StartPackServerOptions): Promise<McpServer> {
