...
describe("injectCanaries", () => {
  ...
  it("skips files that already have the canary", () => {
    write("already.ts", `// BARRY-CANARY-0.1.0-rc.3-abcd1234 — existing\nconst x = 1;\n`);
...
describe("stampCanaries", () => {
  it("replaces placeholders with versioned fingerprint", () => {
    ...
    const count = stampCanaries(fixtureDir, "0.1.0-rc.3", "55fe257b");

    expect(count).toBe(2);
    expect(read("src/index.ts")).toContain("BARRY-CANARY-0.1.0-rc.3-55fe257b");
    expect(read("src/index.ts")).not.toContain(CANARY_PLACEHOLDER);
    expect(read("pack.yaml")).toContain("BARRY-CANARY-0.1.0-rc.3-55fe257b");
  });
  ...
  it("works end-to-end with injectCanaries", () => {
    ...
    const stamped = stampCanaries(fixtureDir, "0.2.0-rc.1", "a1b2c3d4");
    expect(stamped).toBe(3);

    expect(read("src/app.ts")).toContain("BARRY-CANARY-0.2.0-rc.1-a1b2c3d4");
    expect(read("src/app.ts")).not.toContain(CANARY_PLACEHOLDER);
    expect(read("config.yaml")).toContain("BARRY-CANARY-0.2.0-rc.1-a1b2c3d4");
    expect(read("README.md")).toContain("BARRY-CANARY-0.2.0-rc.1-a1b2c3d4");
  });
});
