...
/**
 * Path containment for pack-declared launchd services and jobs.
 *
 * launchd runs these unattended at login, so a manifest must not be able to
 * reach outside its own pack with `../`. Kept here rather than inline in the
 * emitter script so the rule is testable — it is a security boundary, and the
 * only thing standing between a pack manifest and an arbitrary path.
 */
...
export interface ResolveResult<T> {
  /** The item with pack-relative paths made absolute, or null if it escaped. */
  item: T | null;
  /** Relative paths that resolved outside the pack. Non-empty means rejected. */
  escaped: string[];
}
...
/**
 * Resolve an item's pack-relative paths against its pack directory.
 *
 ...
 * pack-local — are held to containment.
 */
export function resolveLaunchdItem<T extends LaunchdItem>(packDir: string, item: T): ResolveResult<T> {
  ...
  // file, which silently produced `<pack>/session` and a job that could never
