...
/**
 * OAuth token status checking for remote MCP packs.
 *
 * OAuth packs (Sentry, Notion) use mcp-remote for browser-based auth.
 * mcp-remote stores tokens in ~/.mcp-auth/mcp-remote-<version>/<md5(url)>_tokens.json.
 * This module checks whether cached tokens exist for a given pack URL,
...
import type { RemotePackSource, PackRegistry } from "./types.js";
 ...
 * until the user authorizes via `barry pack auth <name>` or the pack_auth tool.
 */
...
/**
 * Minimal config shape shared by RemotePackSource (registry) and the MCP
 * server's PackServerConfig, so auth classification works on both.
 */
export interface PackAuthShape {
  url?: string;
  command?: string;
  args?: string[];
  env?: string[];
...
/**
 * True when a pack authenticates with API keys (declared env vars or
 * `--header` args) rather than browser-based OAuth. Running an OAuth flow
 * for these packs is always wrong — the server expects header auth.
 */
export function usesApiKeyAuth(config: PackAuthShape): boolean {
  if (config.env?.length) return true;
  return config.args?.includes("--header") ?? false;
}

/**
 * Env vars a pack's auth depends on: declared `env` plus any `${VAR}`
 * placeholders in its args. Used for actionable "missing key" messages.
 */
export function getDeclaredEnvVars(config: PackAuthShape): string[] {
...
/**
 * Identify OAuth packs: remote packs with a URL but no env vars or API key headers.
 * These use browser-based OAuth via mcp-remote, not API keys from profiles.
 */
export function isOAuthPack(source: RemotePackSource): boolean {
...
export interface OAuthPackStatus {
  name: string;
  url: string;
  hasTokens: boolean;
}
...
/**
 * Check OAuth status for all OAuth packs in a registry.
 * Returns only packs that are OAuth-based (skips API key and local packs).
 */
export function checkOAuthStatus(registry: PackRegistry): OAuthPackStatus[] {
  const results: OAuthPackStatus[] = [];

  for (const [name, source] of Object.entries(registry)) {
    if (source.disabled) continue;
    if (source.type !== "remote") continue;
    if (!isOAuthPack(source)) continue;
...
/**
 * Get OAuth packs that need authorization (no cached tokens).
 */
export function getPacksNeedingAuth(registry: PackRegistry): OAuthPackStatus[] {
