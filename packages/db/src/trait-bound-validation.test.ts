...
import {
  findUnknownBoundNames,
  findDroppedBoundOverrides,
  findMissingDefaultTraits,
  findShellessIdentities,
} from "./trait-bound-validation.js";

const KNOWN = new Set(["readonly", "no-bash", "no-secrets"]);

describe("findUnknownBoundNames", () => {
  it("reports a scope name no scope row defines", () => {
    const offenders = findUnknownBoundNames(
  ...
  it("accepts scopes that exist", () => {
    expect(
      findUnknownBoundNames([{ name: "figma", scopes: ["readonly", "no-bash"] }], KNOWN),
    ).toEqual([]);
  });

  it("ignores traits declaring no scopes", () => {
    expect(findUnknownBoundNames([{ name: "all" }, { name: "coding", scopes: [] }], KNOWN)).toEqual(
      [],
    );
  });
  ...
  it("stays silent when no scopes are known at all", () => {
    expect(findUnknownBoundNames([{ name: "read", scopes: ["readonly"] }], new Set())).toEqual([]);
  });
});

describe("findDroppedBoundOverrides", () => {
  ...
  it("reports an override that drops the shadowed trait's scope", () => {
    const dropped = findDroppedBoundOverrides([
  ...
  it("does not report an override that ADDS a scope", () => {
    expect(
      findDroppedBoundOverrides([{ name: "read" }, { name: "read", scopes: ["readonly"] }]),
    ).toEqual([]);
  });
...
describe("findShellessIdentities", () => {
  ...
  it("reports an identity whose default scope denies Bash", () => {
    expect(
      findShellessIdentities([
        { name: "bux", bound: { name: "no-bash", deniedTools: ["Bash"] } },
      ]),
    ).toEqual([{ identity: "bux", bound: "no-bash" }]);
  });
  ...
  it("reports a Bash denial buried in a longer deniedTools list", () => {
    expect(
      findShellessIdentities([
        { name: "reader", bound: { name: "readonly", deniedTools: ["Bash", "Write", "Edit"] } },
      ]),
    ).toEqual([{ identity: "reader", bound: "readonly" }]);
  });

  it("stays silent for an identity pinning no default scope", () => {
    expect(findShellessIdentities([{ name: "bux", bound: null }])).toEqual([]);
  });
  ...
  it("stays silent for a scope that leaves Bash alone", () => {
    expect(
      findShellessIdentities([
        { name: "careful", bound: { name: "no-secrets", deniedTools: [] } },
        { name: "netless", bound: { name: "no-network" } },
      ]),
    ).toEqual([]);
  });
  ...
  it("matches case-insensitively", () => {
    expect(
      findShellessIdentities([{ name: "bux", bound: { name: "custom", deniedTools: ["bash"] } }]),
    ).toEqual([{ identity: "bux", bound: "custom" }]);
  });

  it("reports each offender when several identities are affected", () => {
    expect(
      findShellessIdentities([
        { name: "a", bound: { name: "no-bash", deniedTools: ["Bash"] } },
        { name: "b", bound: null },
        { name: "c", bound: { name: "readonly", deniedTools: ["Bash", "Write"] } },
      ]),
    ).toEqual([
      { identity: "a", bound: "no-bash" },
      { identity: "c", bound: "readonly" },
    ]);
  });
});
