  ...
  for (const [i, row] of rows.entries()) {
    await db
      ...
      .values({
        ...
        description: null,
        tools: db.fn("jsonb_build_array") as never,
        namespaces: db.fn("jsonb_build_array") as never,
        access: "read",
        skills: db.fn("jsonb_build_array") as never,
        scope: db.fn("jsonb_build_object") as never,
        bag: row.bag,
      })
      .execute();
  }
