...
/**
 * One resolver for every write to `identities.metadata.blocks`.
 *
 ...
 * subpath (`@barry/db/identity-blocks`) so `import { Identities } from "@barry/db"`
...
export interface ResolveBlocksResult {
  ...
  /** Blocks dropping out of the barry that had launchd plists to remove. */
...
export interface ResolveBlocksOptions {
  ...
  /** The barry's current block list, used to compute plist cleanup. */
  previous?: string[];
 ...
 * packages/blocks loader), so a barry with a few unreachable remote blocks
