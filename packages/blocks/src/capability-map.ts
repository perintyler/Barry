...
/**
 * Compile a block's verbs & nouns into a capability map for the session prompt.
 *
 * The map is plain guidance text appended to the system prompt (via the same
 * path as block `instructions`). It gives the agent a cross-block view of what
 * it can DO (verbs) and what it can act ON (nouns) so it can compose a verb
 * from one block with a noun from another — e.g. "debug the virtual-tags" pulls
...
import type { MergedBlockConfig, MergedVerb, MergedNoun } from "./merge.js";

function synonymSuffix(synonyms: string[]): string {
  return synonyms.length ? ` (${synonyms.join(", ")})` : "";
}

/** One line per verb, grouping the blocks that declare it. */
function renderVerbs(verbs: MergedVerb[]): string[] {
  // Group by verb name; keep first-seen synonyms/instruction, collect owners.
  const byName = new Map<string, { synonyms: string[]; instruction: string; blocks: string[] }>();
  for (const v of verbs) {
    const existing = byName.get(v.name);
    if (existing) {
      if (!existing.blocks.includes(v.block)) existing.blocks.push(v.block);
    } else {
      byName.set(v.name, { synonyms: v.synonyms, instruction: v.instruction, blocks: [v.block] });
    }
  }
  const lines: string[] = [];
  for (const [name, g] of byName) {
    const instruction = g.instruction.trim().replace(/\s+/g, " ");
    lines.push(`- ${name}${synonymSuffix(g.synonyms)}: ${instruction}  [${g.blocks.join(", ")}]`);
  }
  return lines;
}

/** A section per noun, owned by its block, listing read/write tools and skills. */
function renderNouns(nouns: MergedNoun[]): string[] {
  const lines: string[] = [];
  for (const n of nouns) {
    const desc = n.description.trim().replace(/\s+/g, " ");
    lines.push(`- ${n.block}/${n.name}${synonymSuffix(n.synonyms)}: ${desc}`);
 ...
 * no verbs and no nouns (so blocks without them cost nothing in the prompt).
 */
export function compileCapabilityMap(config: MergedBlockConfig): string | null {
  ...
  if (config.verbs.length > 0) {
    sections.push(
      [
        ...
        "verb may span blocks, so compose across them.",
  ...
  if (config.nouns.length > 0) {
    sections.push(
      [
        "Nouns — the domain objects you act on. The listed block owns the domain;",
