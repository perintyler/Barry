 ...
 * names — a block manifest's `instructions:` string, a SKILL.md body, an
 ...
 * a fully built, shipped prose primitive that reached ZERO adoption (no block
 ...
 * than in @barry/skills because blocks cannot import skills (skills already
 * depends on blocks); `parseSkillFile` re-exports this so there is one parser.
 */
 ...
 * arriving from a remote block gets identical strict treatment to one on disk.
 */
...
/**
 * Instructions dirs for a block — twin of getSkillsDirs / getActionsDirs, same
 * shape so all three discovery paths stay symmetric.
 */
export function getInstructionsDirs(blockDir: string): string[] {
