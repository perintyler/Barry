...
import type { BlockWithServices } from "../service-ports.js";

/** Small stand-in for the real PORTS table so tests do not track it as it grows. */
const RESERVED = { api: 3854, web: 8429, redis: 6379 };

const block = (name: string, services: Array<{ name: string; port?: number }>): BlockWithServices => ({
  name,
  services,
});
...
describe("collectDeclaredPorts", () => {
  it("collects the ports services declare", () => {
    const declared = collectDeclaredPorts([
      block("bdiff", [{ name: "review", port: 3862 }]),
      block("notes", [{ name: "api", port: 3910 }]),
    ]);

    expect(declared).toEqual([
      { block: "bdiff", service: "review", port: 3862 },
      { block: "notes", service: "api", port: 3910 },
    ]);
  });
  ...
  it("ignores services with no port", () => {
    ...
    const declared = collectDeclaredPorts([
      block("jobs", [{ name: "worker" }, { name: "watcher" }]),
    ]);

    expect(declared).toEqual([]);
  });

  it("tolerates a block with no services at all", () => {
...
describe("findPortConflicts", () => {
  ...
  it("detects a block colliding with a core service", () => {
    const declared = collectDeclaredPorts([block("notes", [{ name: "api", port: 3854 }])]);
  ...
  it("detects two blocks claiming the same port", () => {
    const declared = collectDeclaredPorts([
      block("notes", [{ name: "api", port: 3910 }]),
      block("tasks", [{ name: "web", port: 3910 }]),
    ]);
    const conflicts = findPortConflicts(declared, RESERVED);

    expect(conflicts).toHaveLength(1);
    expect(conflicts[0]).toMatchObject({ port: 3910, kind: "block" });
  ...
  it("reports one conflict per contested port, not one per pair", () => {
    ...
    const declared = collectDeclaredPorts([
      block("a", [{ name: "s", port: 3910 }]),
      block("b", [{ name: "s", port: 3910 }]),
      block("c", [{ name: "s", port: 3910 }]),
    ]);
    const conflicts = findPortConflicts(declared, RESERVED);
  ...
  it("detects one block colliding with itself across two services", () => {
    const declared = collectDeclaredPorts([
      block("notes", [{ name: "api", port: 3910 }, { name: "admin", port: 3910 }]),
    ]);
  ...
  it("prefers the core diagnosis when a port collides with both", () => {
    // The block must move either way, but naming the core service is the
    // actionable half: that port is not negotiable.
    const declared = collectDeclaredPorts([
      block("a", [{ name: "s", port: 3854 }]),
      block("b", [{ name: "s", port: 3854 }]),
    ]);
    const conflicts = findPortConflicts(declared, RESERVED);
  ...
  it("does not offset core ports when checking", () => {
    // Barry derives prod ports by adding 1000 to these bases, but block ports
    ...
    const declared = collectDeclaredPorts([block("notes", [{ name: "api", port: 4854 }])]);
  ...
  it("orders conflicts by port regardless of input order", () => {
    const declared = collectDeclaredPorts([
      block("z", [{ name: "s", port: 9100 }]),
      block("y", [{ name: "s", port: 9100 }]),
      block("a", [{ name: "s", port: 3910 }]),
      block("b", [{ name: "s", port: 3910 }]),
    ]);
  ...
  it("defaults to the real core port table", () => {
    ...
    const declared = collectDeclaredPorts([block("notes", [{ name: "api", port: 6379 }])]);
