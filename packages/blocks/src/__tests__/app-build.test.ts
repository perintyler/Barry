...
describe("resolveAppOutput", () => {
  it("resolves a block-relative bundle path", () => {
    expect(resolveAppOutput(BLOCK, ".build/Demo.app")).toBe("/blocks/demo/.build/Demo.app");
  });

  it("refuses a path that escapes the block", () => {
    expect(resolveAppOutput(BLOCK, "../../etc/Evil.app")).toBeNull();
  });

  it("refuses an absolute path", () => {
    // Unlike a launchd command — where an absolute path deliberately names a
    // host binary — an app bundle is this block's own build artifact. Accepting
    ...
    expect(resolveAppOutput(BLOCK, "/Applications/Evil.app")).toBeNull();
  });

  it("refuses a traversal that lands back inside the block", () => {
    ...
    expect(resolveAppOutput(BLOCK, "../demo/.build/Demo.app")).toBe("/blocks/demo/.build/Demo.app");
  });
});
