...
/**
 * Path containment for bag-declared launchd services and jobs.
 *
 * launchd runs these unattended at login, so a manifest must not be able to
 * reach outside its own bag with `../`. Kept here rather than inline in the
 * emitter script so the rule is testable — it is a security boundary, and the
 * only thing standing between a bag manifest and an arbitrary path.
 */
...
export interface ResolveResult<T> {
  /** The item with bag-relative paths made absolute, or null if it escaped. */
  item: T | null;
  /** Relative paths that resolved outside the bag. Non-empty means rejected. */
  escaped: string[];
}
 ...
 * `--barry` are not. Deliberately conservative — a bare word is never
...
/**
 * Resolve an item's bag-relative paths against its bag directory.
 *
 ...
 * bag-local — are held to containment.
 */
export function resolveLaunchdItem<T extends LaunchdItem>(bagDir: string, item: T): ResolveResult<T> {
  ...
  // file, which silently produced `<bag>/session` and a job that could never
