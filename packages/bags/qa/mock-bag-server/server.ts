...
import { createBagServer } from "../../src/bag-server.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const PORT = 9877;

const bagDir = join(__dirname, "fixture");
  ...
  if (req.url === "/mcp") {
    if (req.method === "POST") {
      ...
      if (isInitializeRequest(parsed)) {
        ...
        const mcpServer = createBagServer({ bagDir });
...
httpServer.listen(PORT, () => {
  ...
  console.warn(`Mock bag server running on http://localhost:${PORT}`);
