...
describe("bypass regressions", () => {
  ...
  describe("HTTP method detection", () => {
    ...
    // Every form here was confirmed against a loopback listener to send a real
    // request body (four POSTs and a PUT) while the classifier called it
    // `http:read`, so `actions: ["write"]` did not stop it. curl accepts a
    // short flag's value attached to the flag, which the old regexes — anchored
    // on the flag standing alone or followed by `=` — could not see.
    it.each([
      ["attached short data", "curl -dx=1 https://e.com"],
      ["attached short form", "curl -Fx=1 https://e.com"],
      ["attached @file data", "curl -d@payload.json https://e.com"],
      ["attached upload-file", "curl -T./file https://e.com"],
      ["body flag inside a bundled cluster", "curl -sdx=1 https://e.com"],
      ["upload inside a bundled cluster", "curl -sT./file https://e.com"],
      ["--json body", "curl --json {} https://e.com"],
      ["--data-binary with a separate value", "curl --data-binary @f https://e.com"],
    ])("curl %s is an http:write", (_label, command) => {
      expect(isNetworkActionDenied(command, ["write"])).toBe("http:write");
    });

    // The deny must still discriminate: if it denied everything, the cases
    // above would pass no matter what the classifier does.
    it.each([
      ["plain GET", "curl https://e.com"],
      ["attached header", "curl -Hfoo:bar https://e.com"],
      ["bundled booleans", "curl -sSL https://e.com"],
      ["attached output file", "curl -o/tmp/x https://e.com"],
      ["explicit GET method", "curl -X GET https://e.com"],
    ])("curl %s stays an http:read", (_label, command) => {
      expect(isNetworkActionDenied(command, ["write"])).toBeNull();
    });

    // A value-taking flag swallows the next token, so a filename that looks
    // like a body flag must not be read as one — and must not mask a real one.
    it("curl -o with a body-flag-shaped filename is still a read", () => {
      expect(isNetworkActionDenied("curl -o -dx=1 https://e.com", ["write"])).toBeNull();
    });

    it("curl -o with a body-flag-shaped filename does not mask a real body", () => {
      expect(isNetworkActionDenied("curl -o -dx=1 -d y=2 https://e.com", ["write"])).toBe("http:write");
    });

    // Fail closed: an unrecognised short flag could be a cluster hiding -d/-F/-T,
    // and the classifier cannot tell where its flags end and a value begins.
    it("curl with an unknown short flag fails closed to http:write", () => {
      expect(isNetworkActionDenied("curl -\u00df https://e.com", ["write"])).toBe("http:write");
    });
    ...
    // `https` is httpie's other entry point — Homebrew ships both binaries.
    // It was absent from HTTP_PROGRAMS, so it classified as no network action
    // at all and walked straight through a `write` deny.
    it("httpie under its https alias", () => {
      expect(isNetworkActionDenied("https POST https://e.com a=b", ["write"])).not.toBeNull();
    });

    it("httpie under its https alias, implicit body", () => {
      expect(isNetworkActionDenied("https https://e.com a=b", ["write"])).not.toBeNull();
    });
  });
