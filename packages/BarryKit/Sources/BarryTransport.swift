...
public struct IdentityTransport: Sendable {
    ...
    /// Edit a bound's description and deny rules. The name is fixed — traits
    /// reference bounds by name, so renaming detaches them.
    public func updateBound(
        id: Int,
        request: Components.Schemas.UpdateBoundRequest
    ) async throws -> Components.Schemas.BoundResponse {
        try await client.updateBound(.init(path: .init(boundId: id), body: .json(request))).ok.body.json
    }
    ...
    public func listBounds() async throws -> Components.Schemas.BoundListResponse {
        try await client.listBounds().ok.body.json
    }

    public func createBound(
        _ request: Components.Schemas.CreateBoundRequest
    ) async throws -> Components.Schemas.BoundResponse {
        let output = try await client.createBound(.init(body: .json(request)))
        return try output.created.body.json
    }
