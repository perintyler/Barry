...
decryption happen in the `@barry-rocks/vault` client, so the server never receives
plaintext item contents.

It exposes:

- `/api/*` for the client library and CLI
- `/mcp` for the built-in Vault bag
- `/` for human secret management

Start it with `barry runtime up`. The CLI `barry vault` commands and barry
