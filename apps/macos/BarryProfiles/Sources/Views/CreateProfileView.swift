...
struct CreateProfileView: View {
    ...
    @State private var allPacks: [PackInfo] = []
    ...
    @State private var packsExpanded = false
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 14) {
                        ...
                        formField("Parent") {
                            ...
                            .pickerStyle(.menu)
                            .labelsHidden()
                            Text("Inherits parent's packs, traits, env, model, vault, and scope")
                                .font(.system(size: 10))
    ...
    private var packsSummary: String {
        let own = selectedPacks.count
        let inherited = parentProfile?.packs.count ?? 0
        if parentId != nil {
    ...
    private var packsContent: some View {
        let inheritedPacks = Set(parentProfile?.packs ?? [])

        // Inherited packs (dimmed, non-interactive)
        if !inheritedPacks.isEmpty {
            ForEach(inheritedPacks.sorted(), id: \.self) { packName in
                HStack(spacing: 10) {
        ...
        // Own packs (interactive)
        ForEach(allPacks.filter { !inheritedPacks.contains($0.name) }) { pack in
            Button {
                if selectedPacks.contains(pack.name) {
                    selectedPacks.remove(pack.name)
                } else {
                    selectedPacks.insert(pack.name)
                }
            } label: {
                HStack(spacing: 10) {
                    checkMark(isChecked: selectedPacks.contains(pack.name))
    ...
    @ViewBuilder
    private func packTypeBadge(_ name: String) -> some View {
        let pack = allPacks.first { $0.name == name }
        Text(pack?.type == "remote" ? "remote" : "local")
            .font(.system(size: 11))
            .foregroundStyle(.tertiary)
    }
