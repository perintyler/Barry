...
(the bdiff block) are scoped to branch/commit contexts, so there's no way to
...
- **Live loop**: the running agent is nudged/polls via the bdiff block
  (`barry bdiff list-comments --session <id>`) and addresses comments mid-flight.
...
- bdiff review service + block (`servers/bdiff`, `blocks/bdiff`); comments table
...
- Comment → session push channel fancier than block polling (e.g. prompt-queue
