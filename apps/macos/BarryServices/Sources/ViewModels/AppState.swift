...
final class AppState: @unchecked Sendable {
    ...
    private func fetchServiceState() async -> [BarryService] {
        ...
        let aliveResults: [(String, Bool, Int?)] = await withTaskGroup(of: (String, Bool, Int?).self) { group in
            for svc in discovered {
                let svcCopy = svc
                group.addTask {
                    let (alive, pid) = isServiceAlive(svcCopy)
                    return (svcCopy.id, alive, pid)
                }
            }
        ...
        let aliveMap = Dictionary(uniqueKeysWithValues: aliveResults.map { ($0.0, ($0.1, $0.2)) })

        // Health check services that are alive and have a port
        let healthTargets = discovered.filter { svc in
            aliveMap[svc.id]?.0 == true && resolvePort(for: svc) != nil
        }
