...
import { summarizeBound, EMPTY_BOUND_SUMMARY } from "./bound-summary.js";

describe("summarizeBound", () => {
  it("names denied tools and access", () => {
    expect(
      summarizeBound({ deniedTools: ["Bash", "Write"], deniedAccess: ["write"] }),
  ...
  it("names network actions and counts denied hosts", () => {
    expect(
      summarizeBound({ network: { actions: ["write"], domains: ["evil.com"] } }),
  ...
  it("reports an empty allowDomains as 'none' rather than omitting it", () => {
    expect(summarizeBound({ network: { allowDomains: [] } })).toBe("allow hosts: none");
  });

  it("counts a populated allowDomains", () => {
    expect(summarizeBound({ network: { allowDomains: ["a.com", "b.com"] } })).toBe(
      "allow hosts: 2",
    );
  });

  it("names the enforcement mode", () => {
    expect(summarizeBound({ network: { enforce: "sandbox" } })).toBe("enforce: sandbox");
  });

  it("joins every dimension in play", () => {
    const summary = summarizeBound({
      deniedTools: ["Bash"],
      files: { deny: ["*.key"] },
  ...
  // to have any — see EMPTY_BOUND_SUMMARY.
  it("reports an empty scope as restricting nothing", () => {
    expect(summarizeBound({})).toBe(EMPTY_BOUND_SUMMARY);
  });

  it("ignores present-but-empty rule arrays", () => {
    expect(
      summarizeBound({ deniedTools: [], deniedAccess: [], files: { deny: [] } }),
    ).toBe(EMPTY_BOUND_SUMMARY);
  });
});
