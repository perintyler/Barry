...
/**
 * Block CLI bridge — dynamically registers block tools as CLI commands.
 *
 * Loading is two-phase:
 *   Phase 1 (fast, every run): read block registry + manifests (YAML only),
 *     register a Commander group per eligible block.
 *   Phase 2 (lazy, on demand): when a block group command is invoked,
 *     dynamically import the block's tools module, build subcommands from
 ...
 * name, the block group is skipped.
 */
...
import { loadRegistry, loadBlock, resolveBlockAccess } from "@barry/blocks";
import type { Block, BlockCliConfig } from "@barry/blocks";
import {
  buildCliSpec,
  formatOutput,
  resolveParams,
...
/**
 * Load the list of block names the user wants as CLI commands.
 *
 ...
 *     - bugbot
 *
 * Returns null if the file doesn't exist (no blocks exposed).
 */
function loadCliConfig(): Set<string> | null {
  ...
  try {
    ...
    // `packs:` is the pre-rename spelling. Existing cli.yaml files still use
    // it, and silently ignoring them exposes no block commands at all — which
    // looks like the blocks are broken rather than the config being stale.
    const list = raw && typeof raw === "object"
      ? (Array.isArray((raw as Record<string, unknown>).blocks)
          ? (raw as Record<string, unknown>).blocks
          : (raw as Record<string, unknown>).packs)
      : undefined;
...
/**
 * Register block command groups with Commander.
 *
 * Phase 1: reads manifests (YAML, no tool imports). Registers a hidden
 * lazy-loading group for each eligible block. The group's action handler
 * triggers Phase 2 on first invocation.
 *
 * Which blocks appear is controlled by ~/.barry/cli.yaml (a list of block names).
 * If the file doesn't exist, no blocks are exposed. Blocks are always reachable
 * via `barry run <block> <tool>` regardless of config.
 */
export function registerBlockGroups(program: Command): void {
  ...
  const enabledBlocks = loadCliConfig();
  if (!enabledBlocks || enabledBlocks.size === 0) return;
  ...
  for (const [name, source] of Object.entries(registry)) {
    ...
    if (!enabledBlocks.has(name)) continue;

    // Load manifest only (no tool imports)
    const result = loadBlock(name);
    // loadBlock can return a Promise for remote blocks with resources — we only
    // care about local blocks here, which resolve synchronously.
    if (result instanceof Promise || !result) continue;
    const block = result;

    if (!block.manifest?.toolsEntry) continue;

    const cliConfig = block.manifest.cli;
    ...
    // can route it to per-tool help in the block dispatcher.
    group.action(async (...actionArgs: unknown[]) => {
      ...
      try {
        await dispatchBlockCommand(block, cliConfig, cmd);
      } finally {
 ...
 */
async function dispatchBlockCommand(
  block: Block,
  cliConfig: BlockCliConfig | undefined,
  cmd: Command,
): Promise<void> {
  const args = cmd.args;
  ...
  if (!subcommand || subcommand === "help" || subcommand === "--help" || subcommand === "--all") {
    await printBlockHelp(block, cliConfig, showAll);
    return;
  }

  // Load tools from the block
  const tools = await loadBlockTools(block);
  if (!tools.length) {
    console.error(`Block "${block.name}" has no CLI-exposed tools.`);
    process.exit(1);
  }
  ...
  if (!matchedTool || !matchedSpec) {
    console.error(`Unknown command: ${block.manifest?.cli?.alias ?? block.name} ${subcommand}`);
    console.error(`Run "barry ${block.manifest?.cli?.alias ?? block.name} help" to see available commands.`);
    process.exit(1);
  }

  // Check for --help on the specific tool
  if (args.includes("--help") || args.includes("-h")) {
    printToolHelp(block, matchedTool, matchedSpec);
    return;
  }
...
/**
 * Import a block's tools module and return all tool definitions.
 */
async function loadBlockTools(block: Block): Promise<ToolDefinition[]> {
  if (!block.manifest?.toolsEntry || block.source.type !== "local") return [];

  const blockDir = resolvePath(block.source.path);
  const entryFile = join(blockDir, block.manifest.toolsEntry.entry);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(`Failed to load tools from block "${block.name}": ${message}`);
    return [];
  }
}
...
/**
 * Print help for all CLI-exposed tools in a block.
 */
async function printBlockHelp(
  block: Block,
  cliConfig: BlockCliConfig | undefined,
  showAll = false,
): Promise<void> {
  const groupName = cliConfig?.alias ?? block.name;
  const tools = await loadBlockTools(block);
 ...
 */
function printToolHelp(block: Block, tool: ToolDefinition, spec: CliToolSpec): void {
  const groupName = block.manifest?.cli?.alias ?? block.name;
...
/**
 * Direct dispatch for `barry run <block> <tool> [args...]`.
 *
 * This is the escape hatch for invoking block tools that are shadowed
...
export async function dispatchBlockTool(
  ...
  const result = loadBlock(blockName);
  if (result instanceof Promise || !result) {
    console.error(`Block "${blockName}" not found or not a local block.`);
    process.exit(1);
  }
  const block = result;

  if (!block.manifest?.toolsEntry) {
    console.error(`Block "${blockName}" has no tools.`);
    process.exit(1);
  }
  ...
  if (!matchedTool || !matchedSpec) {
    console.error(`Unknown tool: ${blockName} ${toolCommand}`);
    process.exit(1);
  }
