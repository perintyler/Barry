...
/**
 * Refuse to emit anything when two services want the same hostname, for the
 * same all-or-nothing reason as the port gate above.
 *
 * Hostnames are the quieter failure of the two. A port collision at least
 * produces an EADDRINUSE from whichever process loses the race; Caddy resolves
 * duplicate site blocks last-one-wins, so a hostname collision reloads cleanly
 * and routes the name at the wrong service forever.
 */
const declaredHosts = collectDeclaredHosts(bags);
const hostConflicts = findHostConflicts(declaredHosts);
if (hostConflicts.length > 0) {
  for (const conflict of hostConflicts) process.stderr.write(conflict.message + "\n");
  process.stderr.write("refusing to emit bag services until hostname conflicts are resolved\n");
  process.exit(1);
}
...
/**
 * Whether a service named a `user:` account that does not exist on this machine.
 *
 * All-or-nothing like the gates above, but for a sharper reason: a service
 * declaring `user:` is asking to run as a DIFFERENT PRINCIPAL, which is the
 * only thing that separates it from the agent sessions sharing this machine.
 * Emitting it anyway would install it as an ordinary LaunchAgent running as the
 * invoking user — indistinguishable from a correctly isolated service, with
 * none of the isolation. The whole point is lost silently, so refuse.
 */
let missingAccount = false;

/** Does this macOS account exist? `id -u` is the cheapest authoritative answer. */
const accountExists = (user: string): boolean => {
  const r = spawnSync("/usr/bin/id", ["-u", user], { stdio: "ignore" });
  return r.status === 0;
};
...
for (const bag of bags) {
  ...
  for (const service of bag.services ?? []) {
    ...
    // A named account must exist BEFORE the service is installed. Creating it
    // is a deliberate, root-requiring install step; silently running as the
    // invoking user instead would defeat the isolation this field exists to
    // provide, while looking identical to a correctly isolated service.
    if (service.user && !accountExists(service.user)) {
      process.stderr.write(
        `${bag.name}.${service.name}: user "${service.user}" does not exist — ` +
          `create the service account before installing this service; ` +
          `refusing to install it as the current user\n`,
      );
      missingAccount = true;
      continue;
    }
    ...
    resources.push({
      ...
      // A socket service has no port to resolve. The registry's `url` carries a
      // unix:// address so a consumer resolves it by name exactly as it would a
      // TCP service, without learning which transport it got.
      ...(service.socket
        ? { url: `unix://${resolve(bagDir(bag.name) ?? ".", service.socket)}` }
        : {}),
      ...(service.health ? { health: service.health } : {}),
      // The pretty local address, when the bag asked for one. Kept beside the
      // loopback url rather than replacing it: this resolves only while Caddy
      // is up, so it is for humans, not for health checks.
      ...(service.host ? { publicUrl: `https://${service.host}` } : {}),
    });
  }
  ...
  for (const app of bag.apps ?? []) {
    // An iOS app has no install destination on this host: it cannot live in
    // /Applications, be launched by bundle id, or run as a launchd agent. It
    // is skipped here rather than in setup so the prune sweep also treats it
    // as undeclared — the same reason a disabled job is skipped above. It is
    // still built by `barry ios build`, which installs it onto a device.
...
/**
 * Caddy routing for bag-declared hostnames, written beside the plists.
 *
 * Always written, even when empty: the main Caddyfile imports this path
 * unconditionally, and Caddy fails a reload on a missing import. An empty
 * generated file is the correct representation of "no bag declares a host".
 */
writeFileSync(getBagHostsCaddyPath(), renderBagHostsCaddyfile(declaredHosts));
...
if (missingAccount) {
  process.stderr.write("refusing to emit bag services until their service accounts exist\n");
  process.exit(1);
}
